...
// Concepts:
//   - VK_EXT_present_timing & vkGetPastPresentationTimingEXT
//   - Display V-Sync Refresh Quantization & Frame Margin Optimization
//   - Dynamic Rendering with Depth Testing & Chronometer Visualizer
// ============================================================================
...
// Generate 3D Chronometer Dial Geometry
void generateChronometerGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    uint16_t baseV = static_cast<uint16_t>(vertices.size());
    const int TICKS = 24;
    const float R_OUTER = 0.75f;
    const float R_INNER = 0.60f;
    const float H = 0.08f;

    // Center hub
    vertices.push_back({ { 0.0f, 0.0f,  H}, {0.9f, 0.9f, 0.9f} }); // 0: Top
    vertices.push_back({ { 0.0f, 0.0f, -H}, {0.2f, 0.2f, 0.2f} }); // 1: Bot

    for (int i = 0; i < TICKS; ++i) {
        float angle = float(i) * 2.0f * vk_math::PI / float(TICKS);
        float r = (i % 2 == 0) ? R_OUTER : R_INNER;
        float x = std::cos(angle) * r;
        float y = std::sin(angle) * r;

        float cr = (i % 2 == 0) ? 1.0f : 0.2f;
        float cg = (i % 2 == 0) ? 0.3f : 0.8f;
        float cb = (i % 2 == 0) ? 0.3f : 1.0f;

        vertices.push_back({ {x, y,  H * 0.5f}, {cr, cg, cb} });
        vertices.push_back({ {x, y, -H * 0.5f}, {cr * 0.5f, cg * 0.5f, cb * 0.5f} });
    }

    for (int i = 0; i < TICKS; ++i) {
        uint16_t curTop = static_cast<uint16_t>(2 + i * 2);
        uint16_t curBot = static_cast<uint16_t>(2 + i * 2 + 1);
        uint16_t nextTop = static_cast<uint16_t>(2 + ((i + 1) % TICKS) * 2);
        uint16_t nextBot = static_cast<uint16_t>(2 + ((i + 1) % TICKS) * 2 + 1);
...
int main() {
    ...
    std::cout << " Concepts: VK_EXT_present_timing, Hardware Presentation Feedback,\n";
    std::cout << "           V-Sync Refresh Quantization, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 64: Present Timing & Frame Pacing (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/pacing_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/pacing_scene.frag.spv");
        ...
        generateChronometerGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering with Present Timing & Smooth Frame Pacing (VK_EXT_present_timing)...\n";
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // Orbiting Chronometer
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.8f, vk_math::Vec3(0.0f, 0.0f, 1.0f)) *
                                 vk_math::Mat4::rotate(0.45f, vk_math::Vec3(1.0f, 0.5f, 0.0f));
        ...
        std::cout << "\nAssignment 64 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 64 Error: " << e.what() << "\n";
        return 1;
    }
