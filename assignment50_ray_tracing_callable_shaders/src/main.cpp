...
// Concepts:
//   - VK_KHR_ray_tracing_pipeline & Callable Shader Execution (executeCallableKHR)
//   - Polymorphic Material BRDF Evaluation (Diffuse, GGX Gold, Clearcoat)
...
struct PushConstants {
    vk_math::Mat4 mvp;
    Vec4 materialParams; // x: materialId, y: roughness
};

// Generate Sphere
void generateSphere(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices, float radius) {
    const int RINGS = 16;
    ...
    for (int r = 0; r <= RINGS; ++r) {
        ...
        for (int s = 0; s <= SECTORS; ++s) {
            ...
            vertices.push_back({ {nx * radius, ny * radius, nz * radius}, {nx, ny, nz} });
        }
    }
...
int main() {
    ...
    std::cout << " Concepts: VK_KHR_ray_tracing_pipeline, executeCallableKHR,\n";
    std::cout << "           Polymorphic BRDF Evaluation, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 50: Ray Tracing Callable Shaders (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/callable_vis.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/callable_vis.frag.spv");
        ...
        generateSphere(vertices, indices, 0.42f);
        ...
        std::cout << "[Render Loop] Rendering Multi-Material Polymorphic BRDF Callable Scene...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment50_ray_tracing_callable_shaders
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment50_ray_tracing_callable_shaders");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment50_ray_tracing_callable_shaders");
            glfwPollEvents();
            ...
            // Render 3 Spheres each invoking a different Callable BRDF material
            float posX[] = { -0.95f, 0.0f, 0.95f };
            for (int m = 0; m < 3; ++m) {
                vk_math::Mat4 model = vk_math::Mat4::translate(vk_math::Vec3(posX[m], 0.0f, 0.0f)) *
                                     vk_math::Mat4::rotate(time * 0.8f, vk_math::Vec3(0.0f, 1.0f, 0.0f));

                PushConstants pc{ proj * view * model, Vec4(float(m), 0.2f, 0.0f, 1.0f) };
        ...
        profiler.exportFoldedFile("flamegraph_assignment50_ray_tracing_callable_shaders.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment50_ray_tracing_callable_shaders.html");
        profiler.exportChromeTraceFile("flamegraph_assignment50_ray_tracing_callable_shaders.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 50 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 50 Error: " << e.what() << "\n";
        return 1;
    }
