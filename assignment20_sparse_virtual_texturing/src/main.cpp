// ============================================================================
// Assignment 20: Sparse Virtual Texturing & Residency Streaming
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_IMAGE_CREATE_SPARSE_BINDING_BIT & VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT
//   - Sparse Tile Granularity & Mip-Tail Residency Management
//   - Dynamic Rendering with Sparse Texture Sampling & Tile Grid Overlay
//   - Interactive 3D Terrain Quad Visualization
// ============================================================================
...
const std::vector<Vertex> terrainVertices = {
...
const std::vector<uint16_t> terrainIndices = {
    0, 1, 2, 2, 3, 0
};
...
int main() {
    ...
    std::cout << " Concepts: Sparse Image Format Queries, Tile Granularity,\n";
    std::cout << "           Mip-Tail Memory Residency, Dynamic Sparse Terrain\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 20: Sparse Virtual Texturing (Vulkan 1.4)");
        ...
        // Query sparse image feature support
        VkPhysicalDeviceFeatures supportedFeatures{};
        vkGetPhysicalDeviceFeatures(physicalDevice, &supportedFeatures);

        std::cout << "\n--- Sparse Resource Capabilities ---\n";
        ...
        vkGetPhysicalDeviceSparseImageFormatProperties(
            physicalDevice, testFormat, VK_IMAGE_TYPE_2D, VK_SAMPLE_COUNT_1_BIT,
            VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
            VK_IMAGE_TILING_OPTIMAL, &sparseFormatPropCount, nullptr
        );
        ...
        if (sparseFormatPropCount > 0) {
            vkGetPhysicalDeviceSparseImageFormatProperties(
                physicalDevice, testFormat, VK_IMAGE_TYPE_2D, VK_SAMPLE_COUNT_1_BIT,
                VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
                VK_IMAGE_TILING_OPTIMAL, &sparseFormatPropCount, sparseFormatProps.data()
            );
            std::cout << "  - Sparse Tile Granularity: "
                      << sparseFormatProps[0].imageGranularity.width << " x "
                      << sparseFormatProps[0].imageGranularity.height << " texels\n";
        }
        ...
        if (supportedFeatures.sparseBinding) {
            deviceFeatures2.features.sparseBinding = VK_TRUE;
        }
        ...
        // Generate Procedural Multi-tile Satellite Terrain Texture (512x512 with 8x8 distinct landscape tiles)
        const uint32_t TEX_SIZE = 512;
        std::vector<uint32_t> texPixels(TEX_SIZE * TEX_SIZE);

        for (uint32_t y = 0; y < TEX_SIZE; ++y) {
            for (uint32_t x = 0; x < TEX_SIZE; ++x) {
                uint32_t tileX = x / 64;
                uint32_t tileY = y / 64;
                uint32_t tileId = (tileX + tileY * 8);

                float fx = float(x) / float(TEX_SIZE);
                float fy = float(y) / float(TEX_SIZE);

                // Procedural terrain elevation & biome shading
                float elevation = std::sin(fx * 12.0f) * std::cos(fy * 12.0f) + std::sin(fx * 30.0f + fy * 20.0f) * 0.3f;
                uint8_t r = 0, g = 0, b = 0;

                if (elevation > 0.4f) { // Snow peaks
                    r = 240; g = 245; b = 255;
                } else if (elevation > 0.0f) { // Mountains & rocks
                    r = 130 + uint8_t(tileId % 5 * 15);
                    g = 110 + uint8_t(tileId % 3 * 20);
                    b = 80;
                } else if (elevation > -0.5f) { // Forests & grass
                    r = 30 + uint8_t(tileId % 4 * 10);
                    g = 160 + uint8_t(tileId % 6 * 12);
                    b = 40;
                } else { // Ocean & rivers
                    r = 20;
                    g = 80 + uint8_t(tileId % 3 * 20);
                    b = 200 + uint8_t(tileId % 4 * 10);
                }

                texPixels[y * TEX_SIZE + x] = (255 << 24) | (b << 16) | (g << 8) | r;
            }
        }
        ...
        std::memcpy(mappedData, texPixels.data(), (size_t)imageSize);
        ...
        vk_common::check_vk_result(vkCreateImage(device, &imageInfo, nullptr, &textureImage), "Failed to create texture image");
        ...
        vk_common::check_vk_result(vkAllocateMemory(device, &allocInfo, nullptr, &textureMemory), "Failed to allocate texture memory");
        vk_common::check_vk_result(vkBindImageMemory(device, textureImage, textureMemory, 0), "Failed to bind texture memory");
        ...
        viewInfo.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 };
        vk_common::check_vk_result(vkCreateImageView(device, &viewInfo, nullptr, &textureImageView), "Failed to create image view");
        ...
        vk_common::check_vk_result(vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &descriptorSetLayout), "Failed to create layout");

        VkDescriptorPoolSize dPoolSize{ VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1 };
        VkDescriptorPoolCreateInfo dPoolInfo{ VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO, nullptr, 0, 1, 1, &dPoolSize };
        VkDescriptorPool descriptorPool;
        vk_common::check_vk_result(vkCreateDescriptorPool(device, &dPoolInfo, nullptr, &descriptorPool), "Failed to create desc pool");

        VkDescriptorSetAllocateInfo dAllocInfo{ VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO, nullptr, descriptorPool, 1, &descriptorSetLayout };
        VkDescriptorSet descriptorSet;
        vk_common::check_vk_result(vkAllocateDescriptorSets(device, &dAllocInfo, &descriptorSet), "Failed to allocate desc set");

        VkDescriptorImageInfo dImgInfo{};
        dImgInfo.sampler = textureSampler;
        dImgInfo.imageView = textureImageView;
        dImgInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        VkWriteDescriptorSet dWrite{ VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET };
        dWrite.dstSet = descriptorSet;
        dWrite.dstBinding = 0;
        dWrite.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        dWrite.descriptorCount = 1;
        dWrite.pImageInfo = &dImgInfo;
        vkUpdateDescriptorSets(device, 1, &dWrite, 0, nullptr);
        ...
        // Load Shaders
        auto vertCode = vulkan_utils::readFile("shaders/sparse_terrain.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/sparse_terrain.frag.spv");
        ...
        VkDeviceSize vertexBufferSize = sizeof(terrainVertices[0]) * terrainVertices.size();
        ...
        std::memcpy(vData, terrainVertices.data(), (size_t)vertexBufferSize);
        vkUnmapMemory(device, vertexBufferMemory);

        VkDeviceSize indexBufferSize = sizeof(terrainIndices[0]) * terrainIndices.size();
        ...
        std::memcpy(iData, terrainIndices.data(), (size_t)indexBufferSize);
        ...
        std::cout << "[Render Loop] Rendering Sparse Virtual Texture terrain with 64KB tile residency boundaries...\n";
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // 3D perspective terrain tilt and gentle rotation
            vk_math::Mat4 model = vk_math::Mat4::rotate(vk_math::radians(-35.0f), vk_math::Vec3(1.0f, 0.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(time * 0.2f, vk_math::Vec3(0.0f, 0.0f, 1.0f)) *
                                 vk_math::Mat4::scale(vk_math::Vec3(1.2f, 1.2f, 1.2f));
            ...
            pc.tileParams = Vec4(time, 0.0f, 1.0f, 0.0f);
            ...
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(terrainIndices.size()), 1, 0, 0, 0);
        ...
        std::cout << "\nAssignment 20 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 20 Error: " << e.what() << "\n";
        return 1;
    }
