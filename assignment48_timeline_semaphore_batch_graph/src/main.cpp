...
// Concepts:
//   - Monotonic 64-bit Timeline Semaphores (VK_SEMAPHORE_TYPE_TIMELINE)
//   - DAG Render Graph Task Orchestration across GPU milestones
//   - Lock-Free Multi-Stage Execution with vkGetSemaphoreCounterValue tracking
...
// Generate 3D Orbiting Satellite Rings + Core Sphere Geometry
void generateSatelliteGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    for (int r = 0; r <= RINGS; ++r) {
        ...
        for (int s = 0; s <= SECTORS; ++s) {
            ...
            vertices.push_back({ {x, y, z}, {1.0f, 0.8f, 0.2f} });
        }
    }
    ...
    // 2. Outer Orbit Ring
    uint16_t ringStart = static_cast<uint16_t>(vertices.size());
    const int SEGMENTS = 32;
    const float RING_R = 0.8f;
    for (int i = 0; i < SEGMENTS; ++i) {
        float a = float(i) * 2.0f * vk_math::PI / float(SEGMENTS);
        float x = std::cos(a) * RING_R;
        float z = std::sin(a) * RING_R;
        vertices.push_back({ {x, -0.05f, z}, {0.2f, 0.9f, 1.0f} });
        vertices.push_back({ {x,  0.05f, z}, {1.0f, 0.3f, 0.8f} });
    }

    for (int i = 0; i < SEGMENTS; ++i) {
        uint16_t i0 = ringStart + static_cast<uint16_t>(i * 2);
        uint16_t i1 = ringStart + static_cast<uint16_t>(i * 2 + 1);
        uint16_t i2 = ringStart + static_cast<uint16_t>(((i + 1) % SEGMENTS) * 2);
        uint16_t i3 = ringStart + static_cast<uint16_t>(((i + 1) % SEGMENTS) * 2 + 1);
...
int main() {
    ...
    std::cout << " Concepts: Monotonic 64-bit Timeline Semaphores (VK_KHR_timeline_semaphore),\n";
    std::cout << "           Render Graph Milestone Dependencies, Lock-Free GPU Progression\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 48: Timeline Semaphores (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/graph_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/graph_scene.frag.spv");
        ...
        // Satellite & Ring Geometry
        std::vector<Vertex> vertices;
        std::vector<uint16_t> indices;
        generateSatelliteGeometry(vertices, indices);
        ...
        // CREATE 64-BIT MONOTONIC TIMELINE SEMAPHORE (initial value = 0)
        ...
        VkSemaphore graphTimelineSemaphore;
        vk_common::check_vk_result(vkCreateSemaphore(device, &timelineSemInfo, nullptr, &graphTimelineSemaphore), "Failed to create timeline semaphore");

        // Binary semaphores for swapchain present
        ...
        std::cout << "[Render Loop] Orchestrating Render Graph tasks with 64-bit Timeline Semaphores...\n";
        ...
        // Initialize Flame Graph Profiler for assignment48_timeline_semaphore_batch_graph
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment48_timeline_semaphore_batch_graph");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment48_timeline_semaphore_batch_graph");
            glfwPollEvents();
            ...
            timelineValue += 1; // Milestone threshold
            ...
            colorAttachment.clearValue = { { { 0.04f, 0.04f, 0.09f, 1.0f } } };
            ...
            // Orbiting Core + Ring Transformation
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.8f, vk_math::Vec3(0.5f, 1.0f, 0.2f)) *
                                 vk_math::Mat4::rotate(time * 0.4f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
            ...
            // SUBMIT WITH TIMELINE SEMAPHORE MONOTONIC PROGRESSION
            VkTimelineSemaphoreSubmitInfo timelineSubmitInfo{};
            timelineSubmitInfo.sType = VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO;
            timelineSubmitInfo.signalSemaphoreValueCount = 1;
            timelineSubmitInfo.pSignalSemaphoreValues = &timelineValue;

            VkSemaphore signalSemaphores[2] = { renderFinishedSemaphore, graphTimelineSemaphore };
            ...
            submitInfo.signalSemaphoreCount = 2; // Signals both binary renderFinished and timeline semaphore
        ...
        profiler.exportFoldedFile("flamegraph_assignment48_timeline_semaphore_batch_graph.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment48_timeline_semaphore_batch_graph.html");
        profiler.exportChromeTraceFile("flamegraph_assignment48_timeline_semaphore_batch_graph.json");
        profiler.cleanupGpu();
        ...
        vkDestroySemaphore(device, graphTimelineSemaphore, nullptr);
        ...
        std::cout << "\nAssignment 48 executed cleanly (" << frameCount << " graph frames scheduled, timeline counter: " << timelineValue << ").\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 48 Error: " << e.what() << "\n";
        return 1;
    }
