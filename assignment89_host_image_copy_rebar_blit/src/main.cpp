...
// Concepts:
//   - VK_EXT_host_image_copy & ReBAR Zero-Copy PCIe Transfers
//   - vkCopyMemoryToImageEXT & Direct Host-Side Layout Transitions
...
void generateQuadGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    for (auto i : boxInd) indices.push_back(base + i);
}
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 89: Host Image Copy & ReBAR Zero-Copy Blit (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_EXT_host_image_copy, ReBAR Direct Host DMA Upload,\n";
    std::cout << "           Zero-Staging-Buffer Texture Streaming & Dynamic Rendering\n";
    std::cout << "====================================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 89: Host Image Copy ReBAR Blit (Vulkan 1.4)");
        ...
        generateQuadGeometry(vertices, indices);
        ...
        std::string shaderDir = "assignment89_host_image_copy_rebar_blit/shaders/";
        if (!std::filesystem::exists(shaderDir + "rebar_blit.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "rebar_blit.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "rebar_blit.frag.spv");
        ...
        std::cout << "[Host Image Copy] Configured direct ReBAR host memory transfer.\n";
        std::cout << "[Render Loop] Executing Zero-Copy Host Image Direct Blit Loop...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        int frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment89_host_image_copy_rebar_blit
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment89_host_image_copy_rebar_blit");
        profiler.initGpu(device, physicalDevice);

        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            VK_PROFILE_SCOPE("assignment89_host_image_copy_rebar_blit");
            glfwPollEvents();
            ...
            colorAttachment.clearValue.color = { {0.04f, 0.04f, 0.08f, 1.0f} };
            ...
            pc.blitParams = Vec4(time, 0.0f, 0.0f, 0.0f);
        ...
        profiler.exportFoldedFile("flamegraph_assignment89_host_image_copy_rebar_blit.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment89_host_image_copy_rebar_blit.html");
        profiler.exportChromeTraceFile("flamegraph_assignment89_host_image_copy_rebar_blit.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 89 executed cleanly (400 frames rendered).\n";
    }
