...
//   - Post-Mortem TDR Diagnosis & vkGetQueueCheckpointDataNV
//   - Dynamic Rendering with Depth Testing & Checkpoint Telemetry
// ============================================================================
...
void generateStarGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    vertices.push_back({ { 0.0f,  R,  0.0f}, {1.0f, 0.2f, 0.2f} }); // Top
    vertices.push_back({ { 0.0f, -R,  0.0f}, {0.2f, 0.2f, 1.0f} }); // Bot
    vertices.push_back({ { R,  0.0f,  0.0f}, {0.2f, 1.0f, 0.2f} }); // Right
    vertices.push_back({ {-R,  0.0f,  0.0f}, {1.0f, 1.0f, 0.2f} }); // Left
    vertices.push_back({ { 0.0f,  0.0f,  R}, {1.0f, 0.2f, 1.0f} }); // Front
    vertices.push_back({ { 0.0f,  0.0f, -R}, {0.2f, 1.0f, 1.0f} }); // Back

    // Inner core
    vertices.push_back({ { r,  r,  r}, {1.0f, 1.0f, 1.0f} });
    vertices.push_back({ {-r,  r,  r}, {1.0f, 1.0f, 1.0f} });
    vertices.push_back({ {-r, -r,  r}, {1.0f, 1.0f, 1.0f} });
    vertices.push_back({ { r, -r,  r}, {1.0f, 1.0f, 1.0f} });
    vertices.push_back({ { r,  r, -r}, {1.0f, 1.0f, 1.0f} });
    ...
    uint16_t faces[] = {
        ...
        1,9,8, 1,8,12, 1,12,13, 1,13,9,
        4,6,9, 4,9,8, 4,8,7, 4,7,6,
        5,10,11, 5,11,12, 5,12,13, 5,13,10,
        2,6,10, 2,10,13, 2,13,9, 2,9,6,
        3,7,8, 3,8,12, 3,12,11, 3,11,7
    };
    for (auto f : faces) indices.push_back(baseV + f);
}
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 54: Device Diagnostic Checkpoints (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_device_diagnostic_checkpoints, Breadcrumbs,\n";
    std::cout << "           Post-Mortem TDR Analysis, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 54: Device Diagnostic Checkpoints (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/diag_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/diag_scene.frag.spv");
        ...
        std::cout << "[Render Loop] Rendering with Diagnostic Breadcrumb Checkpoints (VK_NV_device_diagnostic_checkpoints)...\n";
        ...
        std::cout << "\nAssignment 54 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 54 Error: " << e.what() << "\n";
        return 1;
    }
