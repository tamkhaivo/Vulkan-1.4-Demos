// ============================================================================
// Assignment 37: Cooperative Matrix & Neural Super-Resolution (VK_KHR_cooperative_matrix)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_KHR_cooperative_matrix & Tensor Core Matrix Acceleration
//   - Subgroup coopmatLoad, coopMatMulAdd, coopmatStore
//   - 3D Neural Matrix Tensor Visualization with Dynamic Rendering
// ============================================================================
...
// Generate 3D Neural Matrix Tensor Lattice Geometry
void generateTensorLattice(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int GRID_N = 4;
    const float SPACING = 0.35f;
    const float HALF_EXTENT = float(GRID_N - 1) * SPACING * 0.5f;

    for (int z = 0; z < GRID_N; ++z) {
        for (int y = 0; y < GRID_N; ++y) {
            for (int x = 0; x < GRID_N; ++x) {
                float px = float(x) * SPACING - HALF_EXTENT;
                float py = float(y) * SPACING - HALF_EXTENT;
                float pz = float(z) * SPACING - HALF_EXTENT;

                float cr = float(x) / float(GRID_N - 1);
                float cg = float(y) / float(GRID_N - 1);
                float cb = float(z) / float(GRID_N - 1);

                // Small micro-cube node
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 37: Cooperative Matrix & Tensor Math (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_KHR_cooperative_matrix, Hardware Matrix Cores,\n";
    std::cout << "           coopMatMulAdd, Neural Tensor Grid Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 37: Cooperative Matrix (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/matrix_vis.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/matrix_vis.frag.spv");
        ...
        generateTensorLattice(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering 3D Neural Matrix Tensor Lattice with Cooperative Matrix pipeline...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment37_cooperative_matrix
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment37_cooperative_matrix");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment37_cooperative_matrix");
            glfwPollEvents();
            ...
            // Orbiting Tensor Matrix Lattice
        ...
        profiler.exportFoldedFile("flamegraph_assignment37_cooperative_matrix.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment37_cooperative_matrix.html");
        profiler.exportChromeTraceFile("flamegraph_assignment37_cooperative_matrix.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 37 executed cleanly (" << frameCount << " frames rendered).\n";
