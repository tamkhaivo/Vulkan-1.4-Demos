...
// Concepts:
//   - VK_EXT_mesh_shader, Task Shaders & Mesh Shaders
//   - Frustum Bounding Spheres & Normal Cone Cluster Backface Culling
//   - Continuous Level of Detail (LOD) & SIMD Ballot Compaction
...
// Generate 3D Geosphere mesh with high cluster density
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 30: Mesh Shading Cluster Culling & LOD (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_EXT_mesh_shader, Task Shaders, Cone Culling,\n";
    std::cout << "           Dynamic LOD Morphing & SIMD Subgroup Compaction\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 30: Mesh Shading Cluster Culling (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/cluster_render.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/cluster_render.frag.spv");
        ...
        std::cout << "[Render Loop] Rendering meshlet cluster hierarchy with dynamic LOD and cone culling...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment30_mesh_shading_culling_lod
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment30_mesh_shading_culling_lod");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment30_mesh_shading_culling_lod");
            glfwPollEvents();
            ...
            // Orbiting high-detail meshlet sphere
        ...
        profiler.exportFoldedFile("flamegraph_assignment30_mesh_shading_culling_lod.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment30_mesh_shading_culling_lod.html");
        profiler.exportChromeTraceFile("flamegraph_assignment30_mesh_shading_culling_lod.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 30 executed cleanly (" << frameCount << " meshlet frames rendered).\n";
