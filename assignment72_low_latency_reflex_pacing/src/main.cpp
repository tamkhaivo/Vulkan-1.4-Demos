...
// Concepts:
//   - VK_NV_low_latency2 & vkLatencySleepNV
//   - Click-to-photon latency markers & CPU-GPU frame queue pacing
//   - Dynamic Rendering with Depth Testing & Real-time Telemetry
// ============================================================================
...
// Generate 3D Spinning Hexagonal Prism Geometry
void generateHexPrismGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int sides = 6;
    const float radius = 0.6f;
    const float height = 0.8f;

    uint16_t topCenterIdx = static_cast<uint16_t>(vertices.size());
    vertices.push_back({ { 0.0f, height * 0.5f, 0.0f }, { 1.0f, 1.0f, 1.0f } });

    uint16_t bottomCenterIdx = static_cast<uint16_t>(vertices.size());
    vertices.push_back({ { 0.0f, -height * 0.5f, 0.0f }, { 0.3f, 0.3f, 0.4f } });

    for (int i = 0; i < sides; ++i) {
        float angle = float(i) * 2.0f * 3.14159265f / float(sides);
        ...
        vertices.push_back({ { x, height * 0.5f, z }, { r, g, b } });
        vertices.push_back({ { x, -height * 0.5f, z }, { r * 0.5f, g * 0.5f, b * 0.5f } });
    }

    // Indices for top and bottom caps & sides
    for (int i = 0; i < sides; ++i) {
        int next = (i + 1) % sides;
        uint16_t topCurr = 2 + i * 2;
        uint16_t topNext = 2 + next * 2;
        uint16_t botCurr = 3 + i * 2;
        uint16_t botNext = 3 + next * 2;
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 72: Ultra-Low-Latency Reflex Pacing (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_low_latency2, Sub-Millisecond Input Sampling,\n";
    std::cout << "           Latency Marker Injection & GPU-CPU Frame Pacing\n";
    std::cout << "====================================================================\n";
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Assignment 72: Low Latency Reflex Pacing", nullptr, nullptr);
    if (!window) {
    ...
    try {
        ...
        appInfo.pApplicationName = "Assignment 72 - Low Latency Reflex Pacing";
        ...
        // Configure Dynamic Rendering & Device Features
        ...
        std::string shaderDir = "assignment72_low_latency_reflex_pacing/shaders/";
        if (!std::filesystem::exists(shaderDir + "latency_cube.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "latency_cube.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "latency_cube.frag.spv");
        ...
        generateHexPrismGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Running Low Latency Reflex Paced Frame Loop...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment72_low_latency_reflex_pacing
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment72_low_latency_reflex_pacing");
        profiler.initGpu(device, physicalDevice);

        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            VK_PROFILE_SCOPE("assignment72_low_latency_reflex_pacing");
            // Reflex Pacing Marker 1: Input Sample
            glfwPollEvents();
            ...
            // Reflex Pacing Marker 2: Simulation Update
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 1.8f, { 0.0f, 1.0f, 0.0f }) * vk_math::Mat4::rotate(time * 0.9f, { 1.0f, 0.0f, 0.0f });
            ...
            pc.highlightColor = Vec4(0.2f + 0.8f * std::sin(time * 3.0f), 0.9f, 0.5f + 0.5f * std::cos(time * 2.0f), 1.0f);

            // Reflex Pacing Marker 3: Render Submit
            ...
            colorAttachment.clearValue = {{{ 0.03f, 0.04f, 0.08f, 1.0f }}};
        ...
        profiler.exportFoldedFile("flamegraph_assignment72_low_latency_reflex_pacing.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment72_low_latency_reflex_pacing.html");
        profiler.exportChromeTraceFile("flamegraph_assignment72_low_latency_reflex_pacing.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 72 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 72 Error: " << e.what() << "\n";
        return 1;
    }
