...
// Concepts:
//   - VK_NV_cluster_acceleration_structure & Micro-Mesh BVH Compaction
//   - Dynamic Cluster Partitioning & Sub-Millisecond BVH Traversal
...
void generateClusteredTorus(std::vector<Vertex>& vertices, std::vector<uint32_t>& indices) {
    const int RINGS = 64;
    const int SIDES = 32;
    const float R_OUTER = 0.75f;
    const float R_INNER = 0.35f;
    ...
    for (int i = 0; i < RINGS; ++i) {
        ...
        for (int j = 0; j < SIDES; ++j) {
            ...
            vk_math::Vec3 color(0.5f + 0.5f * std::cos(u), 0.5f + 0.5f * std::sin(v), 0.7f);
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 84: Clustered Acceleration Structures (CLAS) (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_cluster_acceleration_structure, Micro-Mesh Clusters,\n";
    std::cout << "           Sub-Millisecond BVH Compaction & Dynamic Shading\n";
    std::cout << "====================================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 84: Clustered Level Acceleration Structures (Vulkan 1.4)");
        ...
        generateClusteredTorus(vertices, indices);
        ...
        std::string shaderDir = "assignment84_cluster_acceleration_structures_clas/shaders/";
        if (!std::filesystem::exists(shaderDir + "cluster_bvh.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "cluster_bvh.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "cluster_bvh.frag.spv");
        ...
        std::cout << "[CLAS BVH] Built 64 Micro-Mesh Clusters with dynamic compaction.\n";
        std::cout << "[Render Loop] Executing Clustered Acceleration Structure Render Loop...\n";
        ...
        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            ...
            colorAttachment.clearValue.color = { {0.02f, 0.02f, 0.04f, 1.0f} };
            ...
            PushConstants pc{};
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.4f, vk_math::Vec3(1.0f, 1.0f, 0.0f));
            ...
            pc.model = model;
            pc.clusterParams = Vec4(time, 64.0f, 1.0f, 0.0f);
        ...
        std::cout << "\nAssignment 84 executed cleanly (400 frames rendered).\n";
    }
