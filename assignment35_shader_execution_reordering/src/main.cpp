...
// Concepts:
//   - VK_NV_shader_execution_reorder & VK_KHR_ray_tracing_position_fetch
//   - hitObjectNV, reorderThreadNV() & Warp Coherence Optimization
...
// Generate 3D Icosahedron & Ring of Spheres
void generateSceneGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    vertices.push_back({ { 0.0f,  0.6f,  0.0f}, {1.0f, 0.9f, 0.2f} }); // Top (0)
    vertices.push_back({ { 0.0f, -0.6f,  0.0f}, {0.2f, 0.8f, 1.0f} }); // Bottom (1)
    vertices.push_back({ {-0.5f,  0.0f,  0.5f}, {1.0f, 0.2f, 0.8f} }); // FL (2)
    vertices.push_back({ { 0.5f,  0.0f,  0.5f}, {0.2f, 1.0f, 0.5f} }); // FR (3)
    vertices.push_back({ { 0.5f,  0.0f, -0.5f}, {0.8f, 0.3f, 1.0f} }); // BR (4)
    vertices.push_back({ {-0.5f,  0.0f, -0.5f}, {0.2f, 0.4f, 1.0f} }); // BL (5)
    ...
    // 2. Orbiting satellite points
    const int ORBIT_COUNT = 8;
    for (int i = 0; i < ORBIT_COUNT; ++i) {
        float angle = float(i) * 2.0f * vk_math::PI / float(ORBIT_COUNT);
        float ox = std::cos(angle) * 1.0f;
        float oz = std::sin(angle) * 1.0f;

        baseV = static_cast<uint16_t>(vertices.size());
        vertices.push_back({ {ox,  0.15f, oz}, {1.0f, 0.5f, 0.2f} });
        vertices.push_back({ {ox - 0.1f, -0.15f, oz}, {0.2f, 1.0f, 0.5f} });
        vertices.push_back({ {ox + 0.1f, -0.15f, oz}, {0.3f, 0.5f, 1.0f} });
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 35: Shader Execution Reordering (SER) (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_shader_execution_reorder, Warp Divergence Elimination,\n";
    std::cout << "           hitObjectNV, reorderThreadNV(), Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 35: Shader Execution Reordering (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/ser_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/ser_scene.frag.spv");
        ...
        std::cout << "[Render Loop] Rendering complex divergent ray tracing geometry with Shader Execution Reordering (SER)...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment35_shader_execution_reordering
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment35_shader_execution_reordering");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment35_shader_execution_reordering");
            glfwPollEvents();
            ...
            // Orbiting scene
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.9f, vk_math::Vec3(0.3f, 1.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(time * 0.4f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
            vk_math::Mat4 view = vk_math::Mat4::lookAt(vk_math::Vec3(0.0f, 0.0f, 2.6f), vk_math::Vec3(0.0f, 0.0f, 0.0f), vk_math::Vec3(0.0f, 1.0f, 0.0f));
        ...
        profiler.exportFoldedFile("flamegraph_assignment35_shader_execution_reordering.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment35_shader_execution_reordering.html");
        profiler.exportChromeTraceFile("flamegraph_assignment35_shader_execution_reordering.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 35 executed cleanly (" << frameCount << " frames rendered).\n";
