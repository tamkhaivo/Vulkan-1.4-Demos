// ============================================================================
// Assignment 43: Cluster Acceleration Structure (CLAS) (VK_NV_cluster_acceleration_structure)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_NV_cluster_acceleration_structure & Sub-BLAS Hierarchical Acceleration
//   - Micro-second dynamic cluster rebuilds & Nanite-style dynamic LOD
...
// Generate 8 Partitioned Geometric Clusters arranged in a circular formation
void generatePartitionedClusters(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int CLUSTERS = 8;
    const float ORBIT_R = 0.65f;
    const float CLUSTER_SIZE = 0.15f;

    for (int c = 0; c < CLUSTERS; ++c) {
        float angle = float(c) * (2.0f * vk_math::PI / float(CLUSTERS));
        float cx = std::cos(angle) * ORBIT_R;
        float cz = std::sin(angle) * ORBIT_R;

        float cr = 0.5f + 0.5f * std::cos(angle);
        float cg = 0.5f + 0.5f * std::sin(angle);
        float cb = 0.8f;

        uint16_t baseV = static_cast<uint16_t>(vertices.size());

        // Octahedron cluster
        vertices.push_back({ {cx,  CLUSTER_SIZE, cz}, {cr, cg, cb} }); // Top (0)
        vertices.push_back({ {cx, -CLUSTER_SIZE, cz}, {cr * 0.6f, cg * 0.6f, cb * 0.6f} }); // Bot (1)
        vertices.push_back({ {cx - CLUSTER_SIZE, 0.0f, cz + CLUSTER_SIZE}, {cr, cg, cb} }); // FL (2)
        vertices.push_back({ {cx + CLUSTER_SIZE, 0.0f, cz + CLUSTER_SIZE}, {cr, cg, cb} }); // FR (3)
        vertices.push_back({ {cx + CLUSTER_SIZE, 0.0f, cz - CLUSTER_SIZE}, {cr, cg, cb} }); // BR (4)
        vertices.push_back({ {cx - CLUSTER_SIZE, 0.0f, cz - CLUSTER_SIZE}, {cr, cg, cb} }); // BL (5)
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 43: Cluster Acceleration Structure (CLAS) (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_cluster_acceleration_structure, Sub-BLAS Partitioning,\n";
    std::cout << "           Hierarchical BVH Compaction, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 43: Cluster Acceleration Structure (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/clas_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/clas_scene.frag.spv");
        ...
        generatePartitionedClusters(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering Partitioned Sub-BLAS Cluster Geometry (CLAS)...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment43_cluster_acceleration_structure
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment43_cluster_acceleration_structure");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment43_cluster_acceleration_structure");
            glfwPollEvents();
            ...
            // Orbiting CLAS scene
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.8f, vk_math::Vec3(0.0f, 1.0f, 0.0f)) *
        ...
        profiler.exportFoldedFile("flamegraph_assignment43_cluster_acceleration_structure.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment43_cluster_acceleration_structure.html");
        profiler.exportChromeTraceFile("flamegraph_assignment43_cluster_acceleration_structure.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 43 executed cleanly (" << frameCount << " frames rendered).\n";
