...
// Concepts:
//   - VK_NV_displacement_micromap & Sub-Triangle Micro-Displacement Fields
//   - Micro-Mesh Acceleration Structures & Real-time Displacement Evaluation
...
// Generate 3D High Resolution Geosphere for Micro-Mesh Displacement
void generateSphereGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    for (int r = 0; r <= RINGS; ++r) {
        ...
        for (int s = 0; s <= SECTORS; ++s) {
            ...
            vertices.push_back({ {nx * R, ny * R, nz * R}, {nx, ny, nz} });
        }
    }
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 39: Displacement Micromaps (DMM) (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_displacement_micromap, Micro-Mesh Ray Tracing,\n";
    std::cout << "           Sub-Triangle Height Arrays, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 39: Displacement Micromaps (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/dmm_mesh.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/dmm_mesh.frag.spv");
        ...
        std::cout << "[Render Loop] Rendering animated Micro-Mesh displaced geometry with Displacement Micromaps...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment39_displacement_micromaps
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment39_displacement_micromaps");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment39_displacement_micromaps");
            glfwPollEvents();
            ...
            PushConstants pc{ proj * view * model, Vec4(0.08f, time, 0.0f, 1.0f) };
        ...
        profiler.exportFoldedFile("flamegraph_assignment39_displacement_micromaps.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment39_displacement_micromaps.html");
        profiler.exportChromeTraceFile("flamegraph_assignment39_displacement_micromaps.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 39 executed cleanly (" << frameCount << " frames rendered).\n";
