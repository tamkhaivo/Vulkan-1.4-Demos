// ============================================================================
// Assignment 62: Subgroup Cluster Operations (VK_KHR_shader_subgroup_clustered)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_SUBGROUP_FEATURE_CLUSTERED_BIT & subgroupClusteredAdd
//   - Intra-Warp Cluster Reductions (Sizes 2, 4, 8, 16)
//   - Dynamic Rendering with Depth Testing & Multi-Cluster Visualizer
// ============================================================================
...
// Generate 3D Clustered Lattice Grid
void generateLatticeGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int GRID = 4;
    ...
    for (int x = 0; x < GRID; ++x) {
        for (int y = 0; y < GRID; ++y) {
            for (int z = 0; z < GRID; ++z) {
                float cx = (float(x) - float(GRID - 1) * 0.5f) * SPACING;
                float cy = (float(y) - float(GRID - 1) * 0.5f) * SPACING;
                float cz = (float(z) - float(GRID - 1) * 0.5f) * SPACING;

                uint16_t baseV = static_cast<uint16_t>(vertices.size());
                float r = float(x) / float(GRID - 1);
                float g = float(y) / float(GRID - 1);
                float b = float(z) / float(GRID - 1);

                vertices.push_back({ {cx - SIZE, cy - SIZE, cz + SIZE}, {r, g, b} });
                vertices.push_back({ {cx + SIZE, cy - SIZE, cz + SIZE}, {r, g, b} });
                vertices.push_back({ {cx + SIZE, cy + SIZE, cz + SIZE}, {r, g, b} });
                vertices.push_back({ {cx - SIZE, cy + SIZE, cz + SIZE}, {r, g, b} });
                vertices.push_back({ {cx - SIZE, cy - SIZE, cz - SIZE}, {r * 0.7f, g * 0.7f, b * 0.7f} });
                vertices.push_back({ {cx + SIZE, cy - SIZE, cz - SIZE}, {r * 0.7f, g * 0.7f, b * 0.7f} });
                vertices.push_back({ {cx + SIZE, cy + SIZE, cz - SIZE}, {r * 0.7f, g * 0.7f, b * 0.7f} });
                vertices.push_back({ {cx - SIZE, cy + SIZE, cz - SIZE}, {r * 0.7f, g * 0.7f, b * 0.7f} });
...
int main() {
    ...
    std::cout << " Concepts: VK_SUBGROUP_FEATURE_CLUSTERED_BIT, Intra-Warp Reductions,\n";
    std::cout << "           subgroupClusteredAdd, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 62: Subgroup Cluster Ops (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/cluster_vis.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/cluster_vis.frag.spv");
        ...
        generateLatticeGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering Clustered Subgroup 3D Lattice Grid (VK_KHR_shader_subgroup_clustered)...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment62_subgroup_cluster_operations
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment62_subgroup_cluster_operations");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment62_subgroup_cluster_operations");
            glfwPollEvents();
            ...
            // Orbiting Lattice Grid
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.7f, vk_math::Vec3(0.5f, 1.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(time * 0.3f, vk_math::Vec3(0.0f, 0.0f, 1.0f));
        ...
        profiler.exportFoldedFile("flamegraph_assignment62_subgroup_cluster_operations.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment62_subgroup_cluster_operations.html");
        profiler.exportChromeTraceFile("flamegraph_assignment62_subgroup_cluster_operations.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 62 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 62 Error: " << e.what() << "\n";
        return 1;
    }
