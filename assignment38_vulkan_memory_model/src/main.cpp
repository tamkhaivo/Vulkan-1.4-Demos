...
// Concepts:
//   - VK_KHR_vulkan_memory_model & vulkanMemoryModelDeviceScope
//   - Fine-grained Acquire/Release semantics on GPU buffers
//   - Lock-free GPU ring buffer progression & real-time visualization
...
// Generate 3D Helix / Spiral Geometry representing the lock-free circular ring buffer
void generateHelixGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int TURNS = 4;
    const int SEGMENTS_PER_TURN = 32;
    const int TOTAL_SEGMENTS = TURNS * SEGMENTS_PER_TURN;
    const float R = 0.6f;
    const float H_TOTAL = 1.2f;

    for (int i = 0; i <= TOTAL_SEGMENTS; ++i) {
        float t = float(i) / float(TOTAL_SEGMENTS);
        float angle = t * float(TURNS) * 2.0f * vk_math::PI;
        float y = (t - 0.5f) * H_TOTAL;
        ...
        // Create small cross section quad
        vertices.push_back({ {x - 0.04f, y, z - 0.04f}, {cr, cg, cb} });
        vertices.push_back({ {x + 0.04f, y, z + 0.04f}, {cr * 0.8f, cg * 0.8f, cb * 0.8f} });
    }

    for (int i = 0; i < TOTAL_SEGMENTS; ++i) {
        uint16_t i0 = static_cast<uint16_t>(i * 2);
        uint16_t i1 = static_cast<uint16_t>(i * 2 + 1);
        uint16_t i2 = static_cast<uint16_t>((i + 1) * 2);
        uint16_t i3 = static_cast<uint16_t>((i + 1) * 2 + 1);
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 38: Vulkan Memory Model (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_KHR_vulkan_memory_model, Acquire/Release Semantics,\n";
    std::cout << "           Device-Scoped Memory Barriers & Lock-Free Progression\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 38: Vulkan Memory Model (Vulkan 1.4)");
        ...
        vulkan13Features.pNext = &memoryModelFeatures;
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/mem_vis.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/mem_vis.frag.spv");
        ...
        std::cout << "[Render Loop] Visualizing Vulkan Memory Model lock-free circular progression...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment38_vulkan_memory_model
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment38_vulkan_memory_model");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment38_vulkan_memory_model");
            glfwPollEvents();
            ...
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.9f, vk_math::Vec3(0.0f, 1.0f, 0.0f)) *
        ...
        profiler.exportFoldedFile("flamegraph_assignment38_vulkan_memory_model.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment38_vulkan_memory_model.html");
        profiler.exportChromeTraceFile("flamegraph_assignment38_vulkan_memory_model.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 38 executed cleanly (" << frameCount << " frames rendered).\n";
