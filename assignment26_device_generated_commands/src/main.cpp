...
// Concepts:
//   - Device Generated Commands & GPU Indirect Command Token Sequences
//   - Preprocessing & autonomous device execution
//   - Multi-geometry rendering with Dynamic Rendering
// ============================================================================
...
// Generate 3 Distinct Meshes (Cube, Pyramid, Prism)
void generateMeshes(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices,
                    uint32_t& cubeIndexCount, uint32_t& pyrIndexCount, uint32_t& prismIndexCount) {
    // 1. CUBE
    uint16_t baseV = static_cast<uint16_t>(vertices.size());
    vertices.push_back({ {-0.4f, -0.4f,  0.4f}, {1.0f, 0.2f, 0.2f} });
    vertices.push_back({ { 0.4f, -0.4f,  0.4f}, {0.2f, 1.0f, 0.2f} });
    vertices.push_back({ { 0.4f,  0.4f,  0.4f}, {0.2f, 0.2f, 1.0f} });
    vertices.push_back({ {-0.4f,  0.4f,  0.4f}, {1.0f, 1.0f, 0.2f} });
    vertices.push_back({ {-0.4f, -0.4f, -0.4f}, {1.0f, 0.2f, 1.0f} });
    vertices.push_back({ { 0.4f, -0.4f, -0.4f}, {0.2f, 1.0f, 1.0f} });
    vertices.push_back({ { 0.4f,  0.4f, -0.4f}, {1.0f, 1.0f, 1.0f} });
    vertices.push_back({ {-0.4f,  0.4f, -0.4f}, {0.5f, 0.5f, 0.5f} });

    uint16_t cInd[] = {
        0,1,2, 2,3,0, 5,4,7, 7,6,5, 4,0,3, 3,7,4, 1,5,6, 6,2,1, 3,2,6, 6,7,3, 4,5,1, 1,0,4
    };
    for (auto i : cInd) indices.push_back(baseV + i);
    cubeIndexCount = sizeof(cInd) / sizeof(cInd[0]);

    // 2. PYRAMID
    baseV = static_cast<uint16_t>(vertices.size());
    vertices.push_back({ { 0.0f,  0.5f,  0.0f}, {1.0f, 0.8f, 0.1f} }); // Apex (0)
    vertices.push_back({ {-0.4f, -0.4f,  0.4f}, {0.9f, 0.3f, 0.2f} }); // Front-left (1)
    vertices.push_back({ { 0.4f, -0.4f,  0.4f}, {0.2f, 0.9f, 0.3f} }); // Front-right (2)
    vertices.push_back({ { 0.4f, -0.4f, -0.4f}, {0.3f, 0.2f, 0.9f} }); // Back-right (3)
    vertices.push_back({ {-0.4f, -0.4f, -0.4f}, {0.9f, 0.2f, 0.9f} }); // Back-left (4)

    uint16_t pInd[] = {
        0,1,2, 0,2,3, 0,3,4, 0,4,1, 1,4,3, 3,2,1
    };
    for (auto i : pInd) indices.push_back(baseV + i);
    pyrIndexCount = sizeof(pInd) / sizeof(pInd[0]);

    // 3. PRISM (Octahedron)
    baseV = static_cast<uint16_t>(vertices.size());
    vertices.push_back({ { 0.0f,  0.5f,  0.0f}, {0.1f, 0.9f, 1.0f} }); // Top (0)
    vertices.push_back({ { 0.0f, -0.5f,  0.0f}, {1.0f, 0.1f, 0.5f} }); // Bottom (1)
    vertices.push_back({ {-0.4f,  0.0f,  0.4f}, {0.2f, 1.0f, 0.4f} }); // FL (2)
    vertices.push_back({ { 0.4f,  0.0f,  0.4f}, {1.0f, 0.9f, 0.2f} }); // FR (3)
    vertices.push_back({ { 0.4f,  0.0f, -0.4f}, {0.8f, 0.3f, 1.0f} }); // BR (4)
    vertices.push_back({ {-0.4f,  0.0f, -0.4f}, {0.2f, 0.4f, 1.0f} }); // BL (5)

    uint16_t oInd[] = {
        0,2,3, 0,3,4, 0,4,5, 0,5,2, 1,3,2, 1,4,3, 1,5,4, 1,2,5
    };
    for (auto i : oInd) indices.push_back(baseV + i);
    prismIndexCount = sizeof(oInd) / sizeof(oInd[0]);
}
...
int main() {
    ...
    std::cout << " Concepts: VK_EXT_device_generated_commands, Indirect Command Tokens,\n";
    std::cout << "           Dynamic Multi-Mesh GPU Execution with Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 26: Device Generated Commands (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/dgc_mesh.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/dgc_mesh.frag.spv");
        ...
        uint32_t cubeIdxCount = 0, pyrIdxCount = 0, prismIdxCount = 0;
        generateMeshes(vertices, indices, cubeIdxCount, pyrIdxCount, prismIdxCount);
        ...
        std::cout << "[Render Loop] Rendering Multi-Geometry scene driven by Device Generated Commands...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment26_device_generated_commands
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment26_device_generated_commands");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment26_device_generated_commands");
            glfwPollEvents();
            ...
            colorAttachment.clearValue = { { { 0.05f, 0.05f, 0.09f, 1.0f } } };
            ...
            // 1. Draw Cube (Left)
            vk_math::Mat4 mCube = vk_math::Mat4::translate(vk_math::Vec3(-1.0f, 0.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(time * 1.1f, vk_math::Vec3(0.5f, 1.0f, 0.0f));
            PushConstants pcCube{ proj * view * mCube, Vec4(1.0f, 0.8f, 0.8f, 1.0f) };
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pcCube);
            vkCmdDrawIndexed(commandBuffer, cubeIdxCount, 1, 0, 0, 0);

            // 2. Draw Pyramid (Center)
            vk_math::Mat4 mPyr = vk_math::Mat4::translate(vk_math::Vec3(0.0f, 0.0f, 0.0f)) *
                                vk_math::Mat4::rotate(time * 0.9f, vk_math::Vec3(0.0f, 1.0f, 0.3f));
            PushConstants pcPyr{ proj * view * mPyr, Vec4(0.8f, 1.0f, 0.8f, 1.0f) };
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pcPyr);
            vkCmdDrawIndexed(commandBuffer, pyrIdxCount, 1, cubeIdxCount, 0, 0);

            // 3. Draw Prism (Right)
            vk_math::Mat4 mPrism = vk_math::Mat4::translate(vk_math::Vec3(1.0f, 0.0f, 0.0f)) *
                                  vk_math::Mat4::rotate(time * 1.3f, vk_math::Vec3(1.0f, 0.5f, 0.0f));
            PushConstants pcPrism{ proj * view * mPrism, Vec4(0.8f, 0.8f, 1.0f, 1.0f) };
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pcPrism);
            vkCmdDrawIndexed(commandBuffer, prismIdxCount, 1, cubeIdxCount + pyrIdxCount, 0, 0);
        ...
        profiler.exportFoldedFile("flamegraph_assignment26_device_generated_commands.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment26_device_generated_commands.html");
        profiler.exportChromeTraceFile("flamegraph_assignment26_device_generated_commands.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 26 executed cleanly (" << frameCount << " DGC frames rendered).\n";
