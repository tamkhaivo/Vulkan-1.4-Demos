// ============================================================================
// Assignment 41: Multi-View Stereo & VR Rendering (VK_KHR_multiview)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_KHR_multiview & gl_ViewIndex shader matrix indexing
//   - Stereoscopic Interpupillary Distance (IPD) Dual-Eye Parallax Cameras
//   - Dynamic Rendering with Split-Screen Stereo Presentation
//   - Depth Testing and 60+ FPS Real-Time Animation
// ============================================================================
...
// Generate 3D Geosphere geometry for Stereo VR depth perception
void generateSphereGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int RINGS = 18;
    const int SECTORS = 24;
    const float R = 0.6f;

    for (int r = 0; r <= RINGS; ++r) {
        float phi = vk_math::PI * float(r) / float(RINGS);
        for (int s = 0; s <= SECTORS; ++s) {
            float theta = 2.0f * vk_math::PI * float(s) / float(SECTORS);
    ...
    for (int r = 0; r < RINGS; ++r) {
        for (int s = 0; s < SECTORS; ++s) {
            ...
            uint16_t next = static_cast<uint16_t>((r + 1) * (SECTORS + 1) + s);
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 41: Multi-View Stereo & VR Rendering (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_KHR_multiview, Stereo Camera Parallax, gl_ViewIndex,\n";
    std::cout << "           Single Pipeline Dual-Eye Side-by-Side Presentation\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 41: Multi-View Stereo VR (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/vr_multiview.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/vr_multiview.frag.spv");
        ...
        std::cout << "[Render Loop] Rendering Stereoscopic VR Left/Right eye perspectives side-by-side...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment41_multiview_stereo_vr
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment41_multiview_stereo_vr");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment41_multiview_stereo_vr");
            glfwPollEvents();
            ...
            // Compute Left and Right Stereo VR Camera Matrices
            float ipd = 0.07f;
            ...
            vk_math::Mat4 eyeProj = vk_math::Mat4::perspective(vk_math::radians(45.0f), eyeAspect, 0.1f, 100.0f);
        ...
        profiler.exportFoldedFile("flamegraph_assignment41_multiview_stereo_vr.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment41_multiview_stereo_vr.html");
        profiler.exportChromeTraceFile("flamegraph_assignment41_multiview_stereo_vr.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 41 executed cleanly (" << frameCount << " stereo VR frames rendered).\n";
