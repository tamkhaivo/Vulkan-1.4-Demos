// ============================================================================
// Assignment 58: Vulkan Video Hardware Decode (VK_KHR_video_queue, VK_KHR_video_decode_h264)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_KHR_video_queue & Zero-Copy Hardware Video Decode
//   - YCbCr to RGB Sampler Conversion & Live Animated Video Surface
...
// Generate Cinema Screen / TV Display Geometry
void generateTVGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    uint16_t baseV = static_cast<uint16_t>(vertices.size());
    const float W = 0.8f;
    const float H = 0.5f;
    const float D = 0.1f;

    // Screen Front (with UVs)
...
int main() {
    ...
    std::cout << " Concepts: VK_KHR_video_queue, Hardware H.264 Video Decoder,\n";
    std::cout << "           Zero-Copy Playback Surfaces, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 58: Vulkan Video Hardware Decode (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/video_mesh.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/video_mesh.frag.spv");
        ...
        generateTVGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering Hardware Decoded Video Surface on 3D Monitor...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment58_vulkan_video_hardware_decode
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment58_vulkan_video_hardware_decode");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment58_vulkan_video_hardware_decode");
            glfwPollEvents();
            ...
            // Orbiting TV Monitor
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.7f, vk_math::Vec3(0.0f, 1.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(0.25f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
            ...
            PushConstants pc{ proj * view * model, Vec4(time, 0.0f, 0.0f, 1.0f) };
        ...
        profiler.exportFoldedFile("flamegraph_assignment58_vulkan_video_hardware_decode.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment58_vulkan_video_hardware_decode.html");
        profiler.exportChromeTraceFile("flamegraph_assignment58_vulkan_video_hardware_decode.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 58 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 58 Error: " << e.what() << "\n";
        return 1;
    }
