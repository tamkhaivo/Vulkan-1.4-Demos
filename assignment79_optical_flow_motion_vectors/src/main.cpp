...
// Concepts:
//   - VK_NV_optical_flow & Screen-Space Dense Velocity Fields
//   - Temporal Reprojection Motion Vectors (CurMVP vs PrevMVP)
//   - Chromatic Flow Direction & Magnitude Visualization
// ============================================================================
...
struct MotionPushConstants {
    vk_math::Mat4 curMVP;
    vk_math::Mat4 prevMVP;
    vk_math::Mat4 curModel;
};

// Generate 3D Geodesic Multi-Facet Sphere
void generateSphereGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    for (int i = 0; i <= STACKS; ++i) {
        float lat = float(i) * 3.14159265f / float(STACKS) - 3.14159265f / 2.0f;
        float y = RADIUS * std::sin(lat);
        float r = RADIUS * std::cos(lat);

        for (int j = 0; j <= SLICES; ++j) {
            float lon = float(j) * 2.0f * 3.14159265f / float(SLICES);
            float x = r * std::cos(lon);
            float z = r * std::sin(lon);

            vk_math::Vec3 p(x, y, z);
            vertices.push_back({ p, p.normalize() });
        }
    }

    for (int i = 0; i < STACKS; ++i) {
        for (int j = 0; j < SLICES; ++j) {
            uint16_t c0 = i * (SLICES + 1) + j;
            uint16_t c1 = c0 + 1;
            uint16_t n0 = (i + 1) * (SLICES + 1) + j;
            uint16_t n1 = n0 + 1;
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 79: Optical Flow & Motion Vector Estimation (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_optical_flow, Screen-Space Motion Fields, Temporal\n";
    std::cout << "           Velocity Tracking & Dynamic Color-Mapped Reprojection\n";
    std::cout << "====================================================================\n";
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Assignment 79: Optical Flow & Motion Vectors", nullptr, nullptr);
    if (!window) {
    ...
    try {
        ...
        appInfo.pApplicationName = "Assignment 79 - Optical Flow";
        ...
        std::string shaderDir = "assignment79_optical_flow_motion_vectors/shaders/";
        if (!std::filesystem::exists(shaderDir + "flow_field.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "flow_field.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "flow_field.frag.spv");
        ...
        std::cout << "[Render Loop] Executing Optical Flow & Motion Vector Estimation Loop...\n";
        ...
        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            ...
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 2.2f, { 0.0f, 1.0f, 0.0f }) * vk_math::Mat4::rotate(time * 1.1f, { 1.0f, 0.0f, 0.0f });
            ...
            vk_math::Mat4 curMVP = proj * view * model;
            if (frameCount == 0) prevMVP = curMVP;

            MotionPushConstants pc{};
            pc.curMVP = curMVP;
            pc.prevMVP = prevMVP;
            pc.curModel = model;
            ...
            colorAttachment.clearValue = {{{ 0.02f, 0.02f, 0.03f, 1.0f }}};
            ...
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(MotionPushConstants), &pc);
        ...
        std::cout << "\nAssignment 79 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 79 Error: " << e.what() << "\n";
        return 1;
    }
