// ============================================================================
// Assignment 52: Cooperative Vector & Tensor Filtering (VK_KHR_cooperative_matrix)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_KHR_cooperative_matrix & Hardware Tensor Core Convolutions
//   - High-order Image Filter Kernels as Tiled GEMM Operations
...
// Generate 3D Convolution Filter Kernel Grid Matrix
void generateFilterKernelGrid(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int KERNEL_SIZE = 5;
    const float SPACING = 0.28f;
    const float START = -float(KERNEL_SIZE - 1) * SPACING * 0.5f;

    for (int y = 0; y < KERNEL_SIZE; ++y) {
        for (int x = 0; x < KERNEL_SIZE; ++x) {
            float px = START + float(x) * SPACING;
            float py = START + float(y) * SPACING;

            // Gaussian weight curve
            float distSq = (px * px + py * py) / (2.0f * 0.35f * 0.35f);
            float weight = std::exp(-distSq);
            float pz = weight * 0.35f;

            float cr = 0.2f + 0.8f * weight;
            float cg = 0.8f * (1.0f - weight) + 0.2f;
            float cb = 0.9f;

            uint16_t baseV = static_cast<uint16_t>(vertices.size());
            const float S = 0.06f;

            vertices.push_back({ {px - S, py - S, pz + S}, {cr, cg, cb} });
            vertices.push_back({ {px + S, py - S, pz + S}, {cr, cg, cb} });
            vertices.push_back({ {px + S, py + S, pz + S}, {cr, cg, cb} });
            vertices.push_back({ {px - S, py + S, pz + S}, {cr, cg, cb} });
            vertices.push_back({ {px - S, py - S, pz - S}, {cr * 0.7f, cg * 0.7f, cb * 0.7f} });
            vertices.push_back({ {px + S, py - S, pz - S}, {cr * 0.7f, cg * 0.7f, cb * 0.7f} });
            vertices.push_back({ {px + S, py + S, pz - S}, {cr * 0.7f, cg * 0.7f, cb * 0.7f} });
            vertices.push_back({ {px - S, py + S, pz - S}, {cr * 0.7f, cg * 0.7f, cb * 0.7f} });
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 52: Cooperative Vector & Tensor Filtering (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_KHR_cooperative_matrix, Hardware Convolution GEMM,\n";
    std::cout << "           Tensor Core Denoising, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 52: Cooperative Tensor Filtering (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/tensor_vis.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/tensor_vis.frag.spv");
        ...
        generateFilterKernelGrid(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering 3D Tensor Convolution Filter Kernel Matrix...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment52_cooperative_vector_tensor_filtering
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment52_cooperative_vector_tensor_filtering");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment52_cooperative_vector_tensor_filtering");
            glfwPollEvents();
        ...
        profiler.exportFoldedFile("flamegraph_assignment52_cooperative_vector_tensor_filtering.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment52_cooperative_vector_tensor_filtering.html");
        profiler.exportChromeTraceFile("flamegraph_assignment52_cooperative_vector_tensor_filtering.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 52 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 52 Error: " << e.what() << "\n";
        return 1;
    }
