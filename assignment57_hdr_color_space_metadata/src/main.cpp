// ============================================================================
// Assignment 57: HDR Color Space & Mastering Display Metadata (VK_EXT_hdr_metadata)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_EXT_hdr_metadata & VK_COLOR_SPACE_HDR10_ST2084_EXT
//   - BT.2020 Display Primaries, MaxCLL/MaxFALL, and ST 2084 PQ Tonemapping
...
struct PushConstants {
    vk_math::Mat4 mvp;
    Vec4 hdrParams; // x: exposure, yzw: peak luminance color
};

// Generate 3D HDR Crystal Gem Geometry
void generateGemGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    vertices.push_back({ { 0.0f,  R,  0.0f}, {1.5f, 0.4f, 0.3f} }); // Top Apex (0)
    vertices.push_back({ { 0.0f, -R,  0.0f}, {0.3f, 0.8f, 1.8f} }); // Bot Apex (1)
    ...
    for (int i = 0; i < SIDES; ++i) {
        ...
        float cg = 0.8f;
        float cb = 0.5f + 0.5f * std::sin(angle);
        vertices.push_back({ {x, 0.0f, z}, {cr * 2.0f, cg * 2.0f, cb * 2.0f} });
    }

    for (int i = 0; i < SIDES; ++i) {
        uint16_t cur = static_cast<uint16_t>(2 + i);
        uint16_t next = static_cast<uint16_t>(2 + (i + 1) % SIDES);

        indices.push_back(baseV + 0); indices.push_back(baseV + cur); indices.push_back(baseV + next);
        indices.push_back(baseV + 1); indices.push_back(baseV + next); indices.push_back(baseV + cur);
    }
}
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 57: HDR Color Space Management (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_EXT_hdr_metadata, BT.2020 Mastering Primaries,\n";
    std::cout << "           ST 2084 PQ Tonemapping, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 57: HDR10 Color Space (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/hdr_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/hdr_scene.frag.spv");
        ...
        std::cout << "[Render Loop] Rendering HDR10 ST 2084 PQ Tonemapped Crystal Gem...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment57_hdr_color_space_metadata
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment57_hdr_color_space_metadata");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment57_hdr_color_space_metadata");
            glfwPollEvents();
        ...
        profiler.exportFoldedFile("flamegraph_assignment57_hdr_color_space_metadata.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment57_hdr_color_space_metadata.html");
        profiler.exportChromeTraceFile("flamegraph_assignment57_hdr_color_space_metadata.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 57 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 57 Error: " << e.what() << "\n";
        return 1;
    }
