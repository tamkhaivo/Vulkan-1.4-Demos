// ============================================================================
// Assignment 66: Rasterization Order Attachment Access (VK_EXT_rasterization_order_attachment_access)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_EXT_rasterization_order_attachment_access & rasterizationOrderColorAttachmentAccess
//   - Deterministic Per-Pixel Primitive Ordering & Programmable Blending
//   - Dynamic Rendering with Depth Testing & Translucent Layer Compositor
// ============================================================================
...
struct PushConstants {
    vk_math::Mat4 mvp;
    Vec4 layerColor; // rgb: color, a: opacity
};

// Generate 3D Translucent Overlapping Discs Geometry
void generateDiscsGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices,
                           uint32_t& disc1IdxCount, uint32_t& disc2IdxCount, uint32_t& disc3IdxCount) {
    const int SEGS = 32;

    auto addDisc = [&](float radius, float zOffset, vk_math::Vec3 col, uint32_t& count) {
        uint16_t baseV = static_cast<uint16_t>(vertices.size());
        vertices.push_back({ {0.0f, 0.0f, zOffset}, col }); // Center (0)

        for (int i = 0; i <= SEGS; ++i) {
            float angle = float(i) * 2.0f * vk_math::PI / float(SEGS);
            float x = std::cos(angle) * radius;
            float y = std::sin(angle) * radius;
            vertices.push_back({ {x, y, zOffset}, col });
        }

        uint32_t startIdx = static_cast<uint32_t>(indices.size());
        for (int i = 0; i < SEGS; ++i) {
            indices.push_back(baseV + 0);
            indices.push_back(static_cast<uint16_t>(baseV + 1 + i));
            indices.push_back(static_cast<uint16_t>(baseV + 1 + i + 1));
        }
        count = static_cast<uint32_t>(indices.size()) - startIdx;
    };

    addDisc(0.70f,  0.15f, vk_math::Vec3(1.0f, 0.2f, 0.3f), disc1IdxCount);
    addDisc(0.55f,  0.00f, vk_math::Vec3(0.2f, 0.9f, 0.4f), disc2IdxCount);
    addDisc(0.40f, -0.15f, vk_math::Vec3(0.3f, 0.5f, 1.0f), disc3IdxCount);
}
...
int main() {
    ...
    std::cout << "           In-Place Read-Modify-Write, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 66: Rasterization Order Shading (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/raster_order.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/raster_order.frag.spv");
        ...
        rasterizer.cullMode = VK_CULL_MODE_NONE; // Transparent dual-sided
        ...
        depthStencil.depthWriteEnable = VK_FALSE; // Alpha blending transparent pass
        ...
        uint32_t d1Count = 0, d2Count = 0, d3Count = 0;
        generateDiscsGeometry(vertices, indices, d1Count, d2Count, d3Count);
        ...
        std::cout << "[Render Loop] Rendering In-Order Blended Layers (VK_EXT_rasterization_order_attachment_access)...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment66_rasterization_order_subpass_shading
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment66_rasterization_order_subpass_shading");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment66_rasterization_order_subpass_shading");
            glfwPollEvents();
            ...
            // Orbiting View
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.7f, vk_math::Vec3(0.0f, 1.0f, 0.3f)) *
                                 vk_math::Mat4::rotate(0.35f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
            ...
            // Layer 1: Red Disc (Opacity 0.65)
            {
                PushConstants pc{ proj * view * model, Vec4(1.0f, 0.3f, 0.3f, 0.65f) };
                vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc);
                vkCmdDrawIndexed(commandBuffer, d1Count, 1, 0, 0, 0);
            }

            // Layer 2: Green Disc (Opacity 0.65)
            {
                PushConstants pc{ proj * view * model, Vec4(0.3f, 1.0f, 0.4f, 0.65f) };
                vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc);
                vkCmdDrawIndexed(commandBuffer, d2Count, 1, d1Count, 0, 0);
            }

            // Layer 3: Blue Disc (Opacity 0.65)
            {
                PushConstants pc{ proj * view * model, Vec4(0.3f, 0.6f, 1.0f, 0.65f) };
                vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc);
                vkCmdDrawIndexed(commandBuffer, d3Count, 1, d1Count + d2Count, 0, 0);
            }
        ...
        profiler.exportFoldedFile("flamegraph_assignment66_rasterization_order_subpass_shading.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment66_rasterization_order_subpass_shading.html");
        profiler.exportChromeTraceFile("flamegraph_assignment66_rasterization_order_subpass_shading.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 66 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 66 Error: " << e.what() << "\n";
        return 1;
    }
