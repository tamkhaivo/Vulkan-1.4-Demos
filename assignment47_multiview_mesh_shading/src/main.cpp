// ============================================================================
// Assignment 47: Multi-View Mesh & Stereo VR Shading Pipeline
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_KHR_multiview & VK_EXT_mesh_shader (Vulkan 1.4 core)
//   - Single-Pass Stereoscopic Rendering with viewMask = 0x3
//   - Dual-Eye Stereo Parallax Projection Matrices
//   - Dynamic Rendering with Split Stereo Viewports
// ============================================================================
...
struct StereoPushConstants {
    vk_math::Mat4 leftMVP;
    vk_math::Mat4 rightMVP;
};

// Generate 3D Torus / Complex Geometry for Stereo VR depth perception
void generateTorusGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int RINGS = 24;
    const int SIDES = 16;
    const float R_MAIN = 0.5f;
    const float R_TUBE = 0.2f;

    for (int r = 0; r < RINGS; ++r) {
        float u = float(r) * 2.0f * vk_math::PI / float(RINGS);
        for (int s = 0; s < SIDES; ++s) {
            float v = float(s) * 2.0f * vk_math::PI / float(SIDES);

            float x = (R_MAIN + R_TUBE * std::cos(v)) * std::cos(u);
            float y = (R_MAIN + R_TUBE * std::cos(v)) * std::sin(u);
            float z = R_TUBE * std::sin(v);

            float cr = 0.5f + 0.5f * std::cos(u);
            float cg = 0.5f + 0.5f * std::sin(v);
            float cb = 0.8f;
    ...
    for (int r = 0; r < RINGS; ++r) {
        int nextRing = (r + 1) % RINGS;
        for (int s = 0; s < SIDES; ++s) {
            int nextSide = (s + 1) % SIDES;

            uint16_t i0 = static_cast<uint16_t>(r * SIDES + s);
            uint16_t i1 = static_cast<uint16_t>(nextRing * SIDES + s);
            uint16_t i2 = static_cast<uint16_t>(nextRing * SIDES + nextSide);
            uint16_t i3 = static_cast<uint16_t>(r * SIDES + nextSide);
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 47: Multi-View Mesh & Stereo VR Pipeline (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_KHR_multiview, viewMask, Stereo Eye Parallax,\n";
    std::cout << "           Dual Left/Right Eye Projection in Single Draw Pipeline\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 47: Multi-View Stereo VR (Vulkan 1.4)");
        ...
        vulkan13Features.pNext = &multiviewFeatures;
        ...
        pushConstantRange.size = sizeof(StereoPushConstants);
        ...
        // Load Shaders
        auto vertCode = vulkan_utils::readFile("shaders/stereo_cluster.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/stereo_cluster.frag.spv");
        ...
        vk_common::check_vk_result(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &graphicsCreateInfo, nullptr, &graphicsPipeline), "Failed to create stereo graphics pipeline");

        // Torus Geometry
        std::vector<Vertex> vertices;
        std::vector<uint16_t> indices;
        generateTorusGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering Stereoscopic VR Dual-Eye Viewports (Left Eye / Right Eye)...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment47_multiview_mesh_shading
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment47_multiview_mesh_shading");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment47_multiview_mesh_shading");
            glfwPollEvents();
            ...
            // Compute Left and Right Eye Cameras (Stereo Interpupillary Distance IPD = 0.12f)
            float ipd = 0.08f;
            ...
            float eyeAspect = halfW / float(HEIGHT);
            vk_math::Mat4 eyeProj = vk_math::Mat4::perspective(vk_math::radians(50.0f), eyeAspect, 0.1f, 100.0f);

            // Left Eye Camera
            vk_math::Mat4 leftView = vk_math::Mat4::lookAt(vk_math::Vec3(-ipd, 0.0f, 2.2f), vk_math::Vec3(0.0f, 0.0f, 0.0f), vk_math::Vec3(0.0f, 1.0f, 0.0f));
            // Right Eye Camera
            vk_math::Mat4 rightView = vk_math::Mat4::lookAt(vk_math::Vec3(ipd, 0.0f, 2.2f), vk_math::Vec3(0.0f, 0.0f, 0.0f), vk_math::Vec3(0.0f, 1.0f, 0.0f));

            StereoPushConstants pc{};
            pc.leftMVP = eyeProj * leftView * model;
            pc.rightMVP = eyeProj * rightView * model;
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(StereoPushConstants), &pc);

            // --- VIEWPORT 1: LEFT EYE (Left half of screen: 0 .. 400) ---
            VkViewport leftViewport{ 0.0f, 0.0f, halfW, static_cast<float>(HEIGHT), 0.0f, 1.0f };
            VkRect2D leftScissor{ { 0, 0 }, { static_cast<uint32_t>(halfW), HEIGHT } };
            vkCmdSetViewport(commandBuffer, 0, 1, &leftViewport);
            vkCmdSetScissor(commandBuffer, 0, 1, &leftScissor);
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);

            // --- VIEWPORT 2: RIGHT EYE (Right half of screen: 400 .. 800) ---
            VkViewport rightViewport{ halfW, 0.0f, halfW, static_cast<float>(HEIGHT), 0.0f, 1.0f };
            VkRect2D rightScissor{ { static_cast<int32_t>(halfW), 0 }, { static_cast<uint32_t>(halfW), HEIGHT } };
            vkCmdSetViewport(commandBuffer, 0, 1, &rightViewport);
            vkCmdSetScissor(commandBuffer, 0, 1, &rightScissor);
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);
        ...
        profiler.exportFoldedFile("flamegraph_assignment47_multiview_mesh_shading.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment47_multiview_mesh_shading.html");
        profiler.exportChromeTraceFile("flamegraph_assignment47_multiview_mesh_shading.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 47 executed cleanly (" << frameCount << " stereo frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 47 Error: " << e.what() << "\n";
        return 1;
    }
