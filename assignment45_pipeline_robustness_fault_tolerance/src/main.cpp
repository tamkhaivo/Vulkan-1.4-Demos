...
//   - VkPipelineRobustnessCreateInfoEXT for per-pipeline fault isolation
//   - Out-of-bounds safety guarantees & nullDescriptor handling
...
// Generate 3D Crystalline Star Geometry
void generateStarGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const float R_OUT = 0.6f;
    const float R_IN = 0.25f;
    const float H = 0.4f;

    // Top apex (0) and bottom apex (1)
    vertices.push_back({ {0.0f,  H, 0.0f}, {1.0f, 0.9f, 0.2f} });
    vertices.push_back({ {0.0f, -H, 0.0f}, {0.1f, 0.6f, 1.0f} });

    // 10 Star boundary vertices (2..11)
    for (int i = 0; i < 10; ++i) {
        float angle = float(i) * 2.0f * vk_math::PI / 10.0f;
        float r = (i % 2 == 0) ? R_OUT : R_IN;
        float x = std::cos(angle) * r;
        float z = std::sin(angle) * r;
        vk_math::Vec3 col = (i % 2 == 0) ? vk_math::Vec3(1.0f, 0.3f, 0.8f) : vk_math::Vec3(0.2f, 1.0f, 0.8f);
        vertices.push_back({ {x, 0.0f, z}, col });
    }

    for (uint16_t i = 0; i < 10; ++i) {
        uint16_t curr = 2 + i;
        uint16_t next = 2 + ((i + 1) % 10);
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 45: Pipeline Robustness & Fault Tolerance (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_EXT_pipeline_robustness, VkPipelineRobustnessCreateInfoEXT,\n";
    std::cout << "           Granular Out-of-Bounds Memory Safety & Fault Isolation\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 45: Pipeline Robustness (Vulkan 1.4)");
        ...
        VkPhysicalDevicePipelineRobustnessFeaturesEXT robustnessFeatures{};
        robustnessFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PIPELINE_ROBUSTNESS_FEATURES_EXT;
        robustnessFeatures.pipelineRobustness = VK_TRUE;

        VkPhysicalDeviceVulkan13Features vulkan13Features{};
        vulkan13Features.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES;
        vulkan13Features.pNext = &robustnessFeatures;
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/robust_test.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/robust_test.frag.spv");
        ...
        renderingCreateInfo.pNext = &robustnessInfo; // Attached to pipeline create info
        ...
        vk_common::check_vk_result(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &graphicsCreateInfo, nullptr, &graphicsPipeline), "Failed to create robust graphics pipeline");
        ...
        std::cout << "[Render Loop] Rendering Fault-Tolerant Star geometry with Pipeline Robustness...\n";
        ...
        std::cout << "\nAssignment 45 executed cleanly (" << frameCount << " robust frames rendered).\n";
