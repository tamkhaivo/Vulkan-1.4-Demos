// ============================================================================
// Assignment 33: Subgroup Advanced Partitioning & Quad Operations
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_NV_shader_subgroup_partitioned & Subgroup Quad / Ballot operations
//   - Parallel subgroup communication and analytic lane partitioning
//   - 3D Real-time graphics presentation with Dynamic Rendering
// ============================================================================
...
int main() {
    ...
    std::cout << " Concepts: VK_NV_shader_subgroup_partitioned, Subgroup Quad/Ballot,\n";
    std::cout << "           Lock-Free Binning, Dynamic Rendering Presentation\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 33: Subgroup Partitioned (Vulkan 1.4)");
        ...
        std::cout << "Supported Operations: "
                  << ((subgroupProps.supportedOperations & VK_SUBGROUP_FEATURE_QUAD_BIT) ? "QUAD " : "")
                  << ((subgroupProps.supportedOperations & VK_SUBGROUP_FEATURE_BALLOT_BIT) ? "BALLOT " : "")
                  << ((subgroupProps.supportedOperations & VK_SUBGROUP_FEATURE_ARITHMETIC_BIT) ? "ARITHMETIC " : "")
                  << "\n";
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/quad_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/quad_scene.frag.spv");
        ...
        std::cout << "[Render Loop] Rendering animated Subgroup Quad & Partitioned visualizer...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment33_subgroup_partitioned_quad
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment33_subgroup_partitioned_quad");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment33_subgroup_partitioned_quad");
            glfwPollEvents();
        ...
        profiler.exportFoldedFile("flamegraph_assignment33_subgroup_partitioned_quad.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment33_subgroup_partitioned_quad.html");
        profiler.exportChromeTraceFile("flamegraph_assignment33_subgroup_partitioned_quad.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 33 executed cleanly (" << frameCount << " frames rendered).\n";
