// ============================================================================
// Assignment 67: Mesh Shading Multi-Topologies & Multi-Resolution Meshlets (VK_EXT_mesh_shader)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_EXT_mesh_shader & Task-to-Mesh Amplification
//   - Multi-Resolution LOD Meshlets & Dynamic Primitive Topologies
//   - Dynamic Rendering with Depth Testing & Procedural Terrain Wave
// ============================================================================
...
// Generate 3D Procedural Terrain Wave Grid
void generateTerrainGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int W = 28;
    const int H = 28;
    ...
    for (int r = 0; r < H; ++r) {
        for (int c = 0; c < W; ++c) {
            ...
            float y = std::sin(dist * 8.0f) * 0.12f;

            float cr = 0.3f + 0.7f * std::sin(x * 4.0f);
            float cg = 0.6f + 0.4f * std::cos(z * 4.0f);
            float cb = 0.8f;
...
int main() {
    ...
    std::cout << " Concepts: VK_EXT_mesh_shader, Dynamic Meshlet LOD Topologies,\n";
    std::cout << "           Procedural Terrain Wave, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 67: Mesh Shading Multi-Topologies (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/mesh_multi.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/mesh_multi.frag.spv");
        ...
        generateTerrainGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering Multi-Resolution Terrain Meshlets (VK_EXT_mesh_shader)...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment67_mesh_shading_multi_topologies
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment67_mesh_shading_multi_topologies");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment67_mesh_shading_multi_topologies");
            glfwPollEvents();
            ...
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.4f, vk_math::Vec3(0.0f, 1.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(0.6f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
        ...
        profiler.exportFoldedFile("flamegraph_assignment67_mesh_shading_multi_topologies.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment67_mesh_shading_multi_topologies.html");
        profiler.exportChromeTraceFile("flamegraph_assignment67_mesh_shading_multi_topologies.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 67 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 67 Error: " << e.what() << "\n";
        return 1;
    }
