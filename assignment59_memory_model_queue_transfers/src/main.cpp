...
// Concepts:
//   - VK_KHR_vulkan_memory_model & gl_ScopeQueueFamily
//   - Matching Release/Acquire Barrier Pairs (VkBufferMemoryBarrier2)
//   - Timeline Semaphore Multi-Queue Coordination & Dynamic Rendering
// ============================================================================
...
// Generate 3D Double Helix Geometry
void generateHelixGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int STEPS = 40;
    const float R = 0.45f;
    const float H = 1.2f;
    ...
    for (int i = 0; i < STEPS; ++i) {
        ...
        float angle = t * 4.0f * vk_math::PI;
        float y = (t - 0.5f) * H;

        // Strand A
        float xA = std::cos(angle) * R;
        float zA = std::sin(angle) * R;
        vertices.push_back({ {xA, y, zA}, {1.0f, 0.3f, 0.3f} });

        // Strand B
        float xB = std::cos(angle + vk_math::PI) * R;
        float zB = std::sin(angle + vk_math::PI) * R;
        vertices.push_back({ {xB, y, zB}, {0.3f, 0.7f, 1.0f} });

        if (i > 0) {
            uint16_t prevA = static_cast<uint16_t>((i - 1) * 2);
            uint16_t prevB = static_cast<uint16_t>((i - 1) * 2 + 1);
            uint16_t currA = static_cast<uint16_t>(i * 2);
            uint16_t currB = static_cast<uint16_t>(i * 2 + 1);

            // Cross rungs
            if (i % 3 == 0) {
                indices.push_back(currA); indices.push_back(currB); indices.push_back(prevA);
                indices.push_back(prevA); indices.push_back(currB); indices.push_back(prevB);
            }
            indices.push_back(prevA); indices.push_back(currA); indices.push_back(currA);
            indices.push_back(prevB); indices.push_back(currB); indices.push_back(currB);
        }
    }
}
...
int main() {
    ...
    std::cout << "           Matching Release/Acquire Barriers, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 59: Memory Model Transfers (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/queue_render.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/queue_render.frag.spv");
        ...
        rasterizer.cullMode = VK_CULL_MODE_NONE; // View both sides of helix
        ...
        generateHelixGeometry(vertices, indices);

        VkDeviceSize vertexBufferSize = sizeof(vertices[0]) * vertices.size();
        VkBuffer vertexBuffer;
        VkDeviceMemory vertexBufferMemory;
        createBuffer(device, physicalDevice, vertexBufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
        ...
        std::cout << "[Render Loop] Rendering Double Helix with Vulkan Memory Model Queue Ownership Barriers...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment59_memory_model_queue_transfers
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment59_memory_model_queue_transfers");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment59_memory_model_queue_transfers");
            glfwPollEvents();
            ...
            // Memory Model Queue Ownership Acquire Barrier
            VkBufferMemoryBarrier2 acquireBarrier{};
            acquireBarrier.sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER_2;
            acquireBarrier.srcStageMask = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT;
            acquireBarrier.srcAccessMask = VK_ACCESS_2_MEMORY_WRITE_BIT;
            acquireBarrier.dstStageMask = VK_PIPELINE_STAGE_2_VERTEX_ATTRIBUTE_INPUT_BIT;
            acquireBarrier.dstAccessMask = VK_ACCESS_2_VERTEX_ATTRIBUTE_READ_BIT;
            acquireBarrier.srcQueueFamilyIndex = graphicsQueueFamily;
            acquireBarrier.dstQueueFamilyIndex = graphicsQueueFamily;
            acquireBarrier.buffer = vertexBuffer;
            acquireBarrier.offset = 0;
            acquireBarrier.size = VK_WHOLE_SIZE;

            VkDependencyInfo qDep{ VK_STRUCTURE_TYPE_DEPENDENCY_INFO };
            qDep.bufferMemoryBarrierCount = 1;
            qDep.pBufferMemoryBarriers = &acquireBarrier;
            vk14.vkCmdPipelineBarrier2(commandBuffer, &qDep);
            ...
            // Orbiting Double Helix
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.9f, vk_math::Vec3(0.0f, 1.0f, 0.2f)) *
                                 vk_math::Mat4::rotate(0.3f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
        ...
        profiler.exportFoldedFile("flamegraph_assignment59_memory_model_queue_transfers.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment59_memory_model_queue_transfers.html");
        profiler.exportChromeTraceFile("flamegraph_assignment59_memory_model_queue_transfers.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 59 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 59 Error: " << e.what() << "\n";
        return 1;
    }
