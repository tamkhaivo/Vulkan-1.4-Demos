...
// Concepts:
//   - vkQueueBindSparse & Sparse Virtual Memory Commitments
//   - Asynchronous Linear DMA Staging Across Dedicated Transfer Queues
//   - Progressive Residency Streaming with Dynamic Rendering
// ============================================================================
...
// Generate Sparse Voxel Terrain Grid Geometry
void generateSparseVoxelGrid(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int TILES_X = 6;
    const int TILES_Z = 6;
    const float TILE_SIZE = 0.22f;
    const float START_X = -float(TILES_X) * TILE_SIZE * 0.5f;
    const float START_Z = -float(TILES_Z) * TILE_SIZE * 0.5f;

    for (int tz = 0; tz < TILES_Z; ++tz) {
        for (int tx = 0; tx < TILES_X; ++tx) {
            float px = START_X + float(tx) * TILE_SIZE;
            float pz = START_Z + float(tz) * TILE_SIZE;
            float height = 0.15f * std::sin(float(tx) * 0.8f) * std::cos(float(tz) * 0.8f);

            float cr = 0.2f + 0.8f * (float(tx) / float(TILES_X));
            float cg = 0.5f + 0.5f * (float(tz) / float(TILES_Z));
            float cb = 0.9f;

            uint16_t baseV = static_cast<uint16_t>(vertices.size());
            const float S = TILE_SIZE * 0.45f;

            // Voxel Tile Block
            vertices.push_back({ {px - S, height - S, pz + S}, {cr, cg, cb} });
            vertices.push_back({ {px + S, height - S, pz + S}, {cr, cg, cb} });
            vertices.push_back({ {px + S, height + S, pz + S}, {cr, cg, cb} });
            vertices.push_back({ {px - S, height + S, pz + S}, {cr, cg, cb} });
            vertices.push_back({ {px - S, height - S, pz - S}, {cr * 0.7f, cg * 0.7f, cb * 0.7f} });
            vertices.push_back({ {px + S, height - S, pz - S}, {cr * 0.7f, cg * 0.7f, cb * 0.7f} });
            vertices.push_back({ {px + S, height + S, pz - S}, {cr * 0.7f, cg * 0.7f, cb * 0.7f} });
            vertices.push_back({ {px - S, height + S, pz - S}, {cr * 0.7f, cg * 0.7f, cb * 0.7f} });
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 51: Direct DMA Sparse Residency (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: vkQueueBindSparse, 64KB Sparse Virtual Allocations,\n";
    std::cout << "           Async DMA Queues, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 51: Direct DMA Sparse Residency (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/sparse_stream.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/sparse_stream.frag.spv");
        ...
        generateSparseVoxelGrid(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering DMA Sparse Streaming Voxel Grid...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment51_dma_sparse_residency_streaming
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment51_dma_sparse_residency_streaming");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment51_dma_sparse_residency_streaming");
            glfwPollEvents();
            ...
            // Orbiting Terrain Grid
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.6f, vk_math::Vec3(0.0f, 1.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(0.55f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
        ...
        profiler.exportFoldedFile("flamegraph_assignment51_dma_sparse_residency_streaming.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment51_dma_sparse_residency_streaming.html");
        profiler.exportChromeTraceFile("flamegraph_assignment51_dma_sparse_residency_streaming.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 51 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 51 Error: " << e.what() << "\n";
        return 1;
    }
