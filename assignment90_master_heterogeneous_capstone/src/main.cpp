...
//   - Dynamic Rendering with Multi-Light Dynamic PBR Shading
//   - Synchronization2 Auto Pipeline Hazards & Present Layout Barriers
//   - Extended Dynamic State Vectors & Zero Descriptor Pool Allocation
// ============================================================================
...
void generateMasterPolyhedron(std::vector<Vertex>& vertices, std::vector<uint32_t>& indices) {
    const int RINGS = 48;
    const int SIDES = 24;
    ...
    for (int i = 0; i < RINGS; ++i) {
        ...
        for (int j = 0; j < SIDES; ++j) {
            ...
            vk_math::Vec3 color(0.5f + 0.5f * std::cos(u), 0.5f + 0.5f * std::sin(v), 0.85f);
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 90: Master Heterogeneous Engine Capstone II (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Synthesis: 64-bit BDA Pointers, Synchronization2, Dynamic Rendering,\n";
    std::cout << "            Multi-Light PBR Shading & Zero-Descriptor Architecture\n";
    std::cout << "====================================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 90: Master Heterogeneous Mega-Engine (Vulkan 1.4)");
        ...
        generateMasterPolyhedron(vertices, indices);
        ...
        pushConstantRange.size = sizeof(MasterPushConstants);
        ...
        std::string shaderDir = "assignment90_master_heterogeneous_capstone/shaders/";
        if (!std::filesystem::exists(shaderDir + "master_capstone.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "master_capstone.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "master_capstone.frag.spv");
        ...
        std::cout << "[Render Loop] Executing Master Heterogeneous Capstone Mega-Engine Loop...\n";
        ...
        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            ...
            MasterPushConstants pc{};
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.45f, vk_math::Vec3(0.0f, 1.0f, 0.0f));
            ...
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(MasterPushConstants), &pc);
        ...
        std::cout << "\nAssignment 90 executed cleanly (400 frames rendered).\n";
    }
