...
// Concepts:
//   - VK_KHR_fragment_shading_rate & Attachment-Driven Density Maps
//   - Dynamic Gaze Center Tracking & Multi-Tiered Shading Rates (1x1, 2x2, 4x4)
...
void generateSphereGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    for (int i = 0; i <= STACKS; ++i) {
        float stackAngle = 3.14159265f / 2.0f - float(i) * 3.14159265f / float(STACKS);
        float xy = R * std::cos(stackAngle);
        float z = R * std::sin(stackAngle);

        for (int j = 0; j <= SECTORS; ++j) {
            float sectorAngle = float(j) * 2.0f * 3.14159265f / float(SECTORS);
            float x = xy * std::cos(sectorAngle);
            float y = xy * std::sin(sectorAngle);

            vk_math::Vec3 p(x, y, z);
            vk_math::Vec3 normal = p.normalize();
            vk_math::Vec3 color(0.5f + 0.5f * normal.x, 0.5f + 0.5f * normal.y, 0.5f + 0.5f * normal.z);
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 83: Dynamic Fragment Shading Rate & Foveation (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_KHR_fragment_shading_rate, Eye-Gaze Density Maps,\n";
    std::cout << "           Variable Rate Shading Optimization & Dynamic Rendering\n";
    std::cout << "====================================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 83: Dynamic Foveated VRS (Vulkan 1.4)");
        ...
        createBuffer(device, physicalDevice, sizeof(uint16_t) * indices.size(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, indexBuffer, indexMemory);
        ...
        std::string shaderDir = "assignment83_dynamic_foveated_vrs/shaders/";
        if (!std::filesystem::exists(shaderDir + "foveated_vrs.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "foveated_vrs.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "foveated_vrs.frag.spv");
        ...
        std::cout << "[Foveated VRS] Active Gaze Reticle tracking enabled.\n";
        std::cout << "[Render Loop] Executing Foveated Variable Rate Shading Render Loop...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        int frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment83_dynamic_foveated_vrs
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment83_dynamic_foveated_vrs");
        profiler.initGpu(device, physicalDevice);

        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            VK_PROFILE_SCOPE("assignment83_dynamic_foveated_vrs");
            glfwPollEvents();
            ...
            colorAttachment.clearValue.color = { {0.03f, 0.03f, 0.06f, 1.0f} };
            ...
            PushConstants pc{};
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.5f, vk_math::Vec3(0.0f, 1.0f, 0.0f));
            ...
            // Dynamic moving gaze point (simulating XR eye tracking)
            float gazeX = 0.5f + 0.25f * std::cos(time * 1.2f);
            float gazeY = 0.5f + 0.20f * std::sin(time * 1.5f);
            pc.gazePos = Vec4(gazeX, gazeY, time, 0.0f);
        ...
        profiler.exportFoldedFile("flamegraph_assignment83_dynamic_foveated_vrs.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment83_dynamic_foveated_vrs.html");
        profiler.exportChromeTraceFile("flamegraph_assignment83_dynamic_foveated_vrs.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 83 executed cleanly (400 frames rendered).\n";
    }
