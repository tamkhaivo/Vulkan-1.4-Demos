...
//   - Any-Hit-Free alpha testing & micro-triangle cutout evaluation
//   - Dynamic Rendering with Depth Testing & Alpha Masking
// ============================================================================
...
// Generate multi-plane foliage card geometry
void generateFoliageGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int CARD_COUNT = 6;
    const float SIZE = 0.8f;

    for (int i = 0; i < CARD_COUNT; ++i) {
        float angle = float(i) * vk_math::PI / float(CARD_COUNT);
        float ca = std::cos(angle) * (SIZE * 0.5f);
        float sa = std::sin(angle) * (SIZE * 0.5f);

        uint16_t baseV = static_cast<uint16_t>(vertices.size());

        vk_math::Vec3 col = (i % 2 == 0) ? vk_math::Vec3(0.2f, 0.9f, 0.3f) : vk_math::Vec3(0.4f, 1.0f, 0.5f);

        vertices.push_back({ {-ca, -SIZE * 0.5f, -sa}, col });
        vertices.push_back({ { ca, -SIZE * 0.5f,  sa}, col });
        vertices.push_back({ { ca,  SIZE * 0.5f,  sa}, col });
        vertices.push_back({ {-ca,  SIZE * 0.5f, -sa}, col });
...
int main() {
    ...
    std::cout << " Concepts: VK_EXT_opacity_micromap, Micro-Opacity Arrays,\n";
    std::cout << "           Any-Hit Elimination & Dynamic Alpha Cutout Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 32: Opacity Micromaps (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/omm_foliage.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/omm_foliage.frag.spv");
        ...
        generateFoliageGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering foliage with Opacity Micromaps cutout acceleration...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment32_opacity_micromaps
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment32_opacity_micromaps");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment32_opacity_micromaps");
            glfwPollEvents();
            ...
            colorAttachment.clearValue = { { { 0.04f, 0.06f, 0.05f, 1.0f } } };
        ...
        profiler.exportFoldedFile("flamegraph_assignment32_opacity_micromaps.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment32_opacity_micromaps.html");
        profiler.exportChromeTraceFile("flamegraph_assignment32_opacity_micromaps.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 32 executed cleanly (" << frameCount << " frames rendered).\n";
