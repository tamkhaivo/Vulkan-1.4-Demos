...
//   - Deterministic Multi-Layer Transparency & In-Order Subpass Shading
//   - Dynamic Rendering with Alpha Blending & Synchronization2
// ============================================================================
...
void generateMultiLayerPlates(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int LAYERS = 4;
    const float W = 0.6f;
    const float H = 0.6f;

    float layerColors[4][4] = {
        {1.0f, 0.2f, 0.2f, 0.45f}, // Red
        {0.2f, 1.0f, 0.3f, 0.45f}, // Green
        {0.2f, 0.4f, 1.0f, 0.45f}, // Blue
        {1.0f, 0.9f, 0.2f, 0.45f}  // Yellow
    };

    for (int l = 0; l < LAYERS; ++l) {
        uint16_t base = static_cast<uint16_t>(vertices.size());
        float z = -0.4f + float(l) * 0.25f;
        float angle = float(l) * 0.4f;

        float c = std::cos(angle);
        float s = std::sin(angle);

        vk_math::Vec3 p0(-W * c, -H, z - W * s);
        vk_math::Vec3 p1( W * c, -H, z + W * s);
        vk_math::Vec3 p2( W * c,  H, z + W * s);
        vk_math::Vec3 p3(-W * c,  H, z - W * s);
        vk_math::Vec3 norm(s, 0.0f, c);

        Vertex v0{ p0, 0.0f, norm, 0.0f, {layerColors[l][0], layerColors[l][1], layerColors[l][2], layerColors[l][3]} };
        Vertex v1{ p1, 0.0f, norm, 0.0f, {layerColors[l][0], layerColors[l][1], layerColors[l][2], layerColors[l][3]} };
        Vertex v2{ p2, 0.0f, norm, 0.0f, {layerColors[l][0], layerColors[l][1], layerColors[l][2], layerColors[l][3]} };
        Vertex v3{ p3, 0.0f, norm, 0.0f, {layerColors[l][0], layerColors[l][1], layerColors[l][2], layerColors[l][3]} };
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 87: Raster Order Attachment Access & OIT (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_EXT_rasterization_order_attachment_access,\n";
    std::cout << "           Deterministic Multi-Layer Alpha Compositing & Lock-Free OIT\n";
    std::cout << "====================================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 87: Raster Order Attachment Access (Vulkan 1.4)");
        ...
        generateMultiLayerPlates(vertices, indices);
        ...
        std::string shaderDir = "assignment87_raster_order_attachment_oit/shaders/";
        if (!std::filesystem::exists(shaderDir + "raster_order_oit.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "raster_order_oit.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "raster_order_oit.frag.spv");
        ...
        depthStencil.depthWriteEnable = VK_FALSE; // Alpha blending transparent write
        depthStencil.depthCompareOp = VK_COMPARE_OP_LESS_OR_EQUAL;

        // In-Order Alpha Blending configuration
        ...
        std::cout << "[Raster Order OIT] Programmable in-order attachment blending active.\n";
        std::cout << "[Render Loop] Executing Multi-Layer In-Order Transparent Compositing Loop...\n";
        ...
        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            ...
            PushConstants pc{};
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.35f, vk_math::Vec3(0.0f, 1.0f, 0.0f));
            ...
            pc.model = model;
            pc.blendParams = Vec4(time, 4.0f, 0.0f, 0.0f);
        ...
        std::cout << "\nAssignment 87 executed cleanly (400 frames rendered).\n";
    }
