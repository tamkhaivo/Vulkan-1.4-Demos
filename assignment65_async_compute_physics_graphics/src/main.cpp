...
// Concepts:
//   - Dedicated Async Compute Queue & Graphics Queue Orchestration
//   - 64-bit Timeline Semaphores & Cross-Queue Ownership Transfers
//   - Dynamic Rendering with Depth Testing & Real-time Cloth Physics Visualizer
// ============================================================================
...
// Generate 3D Cloth Physics Mesh Grid (30x30)
void generateClothGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const int GRID = 30;
    const float SIZE = 1.4f;
    const float STEP = SIZE / float(GRID - 1);

    for (int r = 0; r < GRID; ++r) {
        for (int c = 0; c < GRID; ++c) {
            float x = -SIZE * 0.5f + float(c) * STEP;
            float z = -SIZE * 0.5f + float(r) * STEP;
            float y = 0.0f;

            float cr = 0.2f + 0.8f * (float(c) / float(GRID - 1));
            float cg = 0.4f + 0.6f * (float(r) / float(GRID - 1));
            float cb = 0.9f;
    ...
    for (int r = 0; r < GRID - 1; ++r) {
        for (int c = 0; c < GRID - 1; ++c) {
            uint16_t cur = static_cast<uint16_t>(r * GRID + c);
            uint16_t next = static_cast<uint16_t>((r + 1) * GRID + c);
...
int main() {
    ...
    std::cout << " Concepts: Multi-Queue Concurrency, Timeline Semaphores,\n";
    std::cout << "           Async Physics Overlap, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 65: Async Compute Physics (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/cloth_render.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/cloth_render.frag.spv");
        ...
        rasterizer.cullMode = VK_CULL_MODE_NONE; // Dual-sided cloth
        ...
        // Timeline Semaphore Setup (64-bit multi-queue synchronization)
        VkSemaphoreTypeCreateInfo timelineInfo{};
        timelineInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_TYPE_CREATE_INFO;
        timelineInfo.semaphoreType = VK_SEMAPHORE_TYPE_TIMELINE;
        timelineInfo.initialValue = 0;

        VkSemaphoreCreateInfo scInfoTimeline{};
        scInfoTimeline.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
        scInfoTimeline.pNext = &timelineInfo;

        VkSemaphore physicsTimelineSemaphore;
        vk_common::check_vk_result(vkCreateSemaphore(device, &scInfoTimeline, nullptr, &physicsTimelineSemaphore), "Failed to create timeline semaphore");
        ...
        std::cout << "[Render Loop] Simulating Real-time Cloth Physics with Timeline Semaphore Sync...\n";
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.4f, vk_math::Vec3(0.0f, 1.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(0.65f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
            ...
            PushConstants pc{ proj * view * model, Vec4(time, 1.0f, 1.0f, 1.0f) };
        ...
        std::cout << "\nAssignment 65 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 65 Error: " << e.what() << "\n";
        return 1;
    }
