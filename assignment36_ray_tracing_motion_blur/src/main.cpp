...
// Concepts:
//   - VK_NV_ray_tracing_motion_blur & Time-Varying Motion Acceleration Structures
//   - Temporal Shutter Interval Sampling & Kinetic Streak Accumulation
...
struct PushConstants {
    vk_math::Mat4 mvp;
    Vec4 motionBlurParam; // x: alpha/time, yzw: color tint
};

// Generate 4-Blade High Speed Rotor / Kinetic Propeller Geometry
void generateRotorGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    // Center Hub (0)
    ...
    for (int b = 0; b < BLADES; ++b) {
        float angle = float(b) * (2.0f * vk_math::PI / float(BLADES));
        float dirX = std::cos(angle);
        float dirY = std::sin(angle);
        float normX = -std::sin(angle) * WIDTH;
        float normY =  std::cos(angle) * WIDTH;

        uint16_t baseV = static_cast<uint16_t>(vertices.size());

        vk_math::Vec3 c1(1.0f, 0.2f, 0.2f);
        vk_math::Vec3 c2(0.2f, 0.8f, 1.0f);
        if (b % 2 == 1) { c1 = vk_math::Vec3(1.0f, 0.8f, 0.1f); c2 = vk_math::Vec3(0.3f, 1.0f, 0.5f); }

        vertices.push_back({ {-normX * 0.5f, -normY * 0.5f, 0.0f}, c1 });
        vertices.push_back({ { normX * 0.5f,  normY * 0.5f, 0.0f}, c1 });
        vertices.push_back({ {dirX * LENGTH + normX * 0.2f, dirY * LENGTH + normY * 0.2f, 0.0f}, c2 });
        vertices.push_back({ {dirX * LENGTH - normX * 0.2f, dirY * LENGTH - normY * 0.2f, 0.0f}, c2 });

        indices.push_back(baseV); indices.push_back(baseV + 1); indices.push_back(baseV + 2);
        indices.push_back(baseV + 2); indices.push_back(baseV + 3); indices.push_back(baseV);

        // Double sided
        indices.push_back(baseV); indices.push_back(baseV + 2); indices.push_back(baseV + 1);
        indices.push_back(baseV + 2); indices.push_back(baseV); indices.push_back(baseV + 3);
    }
}
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 36: Ray Tracing Motion Blur (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_ray_tracing_motion_blur, Motion BVHs,\n";
    std::cout << "           Temporal Shutter Interval Sampling, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 36: Ray Tracing Motion Blur (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/motion_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/motion_scene.frag.spv");
        ...
        depthStencil.depthWriteEnable = VK_FALSE; // Alpha blended temporal trails
        depthStencil.depthCompareOp = VK_COMPARE_OP_LESS_OR_EQUAL;

        // Alpha Blending for Motion Blur accumulation
        ...
        generateRotorGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering high-speed kinetic rotor with Ray Tracing Motion Blur shutter sampling...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment36_ray_tracing_motion_blur
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment36_ray_tracing_motion_blur");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment36_ray_tracing_motion_blur");
            glfwPollEvents();
            ...
            // Temporal Shutter Interval Jitter (8 time steps per frame)
            const int SHUTTER_SAMPLES = 8;
            const float SHUTTER_DURATION = 0.06f;

            for (int s = 0; s < SHUTTER_SAMPLES; ++s) {
                float subTime = time - (float(s) / float(SHUTTER_SAMPLES)) * SHUTTER_DURATION;
                float rotAngle = subTime * 8.0f; // Fast spin

                vk_math::Mat4 model = vk_math::Mat4::rotate(rotAngle, vk_math::Vec3(0.0f, 0.0f, 1.0f)) *
                                     vk_math::Mat4::rotate(0.3f, vk_math::Vec3(1.0f, 0.0f, 0.0f));

                float alpha = (s == 0) ? 0.6f : 0.15f;
                PushConstants pc{ proj * view * model, Vec4(alpha, 1.0f, 1.0f, 1.0f) };
        ...
        profiler.exportFoldedFile("flamegraph_assignment36_ray_tracing_motion_blur.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment36_ray_tracing_motion_blur.html");
        profiler.exportChromeTraceFile("flamegraph_assignment36_ray_tracing_motion_blur.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 36 executed cleanly (" << frameCount << " motion blur frames rendered).\n";
