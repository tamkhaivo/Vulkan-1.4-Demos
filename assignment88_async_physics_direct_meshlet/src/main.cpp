...
// Concepts:
//   - Multi-Queue Timeline Synchronization & Buffer Memory Barriers
//   - Dynamic Physics Simulation Streaming directly into Graphics Pipeline
...
void generateClothGrid(std::vector<Vertex>& vertices, std::vector<uint32_t>& indices) {
    const int GRID = 48;
    ...
    for (int i = 0; i <= GRID; ++i) {
        ...
        for (int j = 0; j <= GRID; ++j) {
            ...
            vk_math::Vec3 p(x, 0.0f, z);
            vk_math::Vec3 normal(0.0f, 1.0f, 0.0f);
            vk_math::Vec3 color(0.2f, 0.6f, 0.95f);
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 88: Async Physics to Direct Meshlet Stream (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: Multi-Queue Timeline Synchronization, Dynamic Cloth Mesh,\n";
    std::cout << "           Async Compute Handoff & Synchronization2 Dynamic Rendering\n";
    std::cout << "====================================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 88: Async Physics Meshlet Stream (Vulkan 1.4)");
        ...
        generateClothGrid(vertices, indices);
        ...
        std::string shaderDir = "assignment88_async_physics_direct_meshlet/shaders/";
        if (!std::filesystem::exists(shaderDir + "physics_meshlet.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "physics_meshlet.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "physics_meshlet.frag.spv");
        ...
        std::cout << "[Timeline Sync] Multi-Queue Handoff timeline semaphore initialized.\n";
        std::cout << "[Render Loop] Executing Async Physics to Meshlet Stream Loop...\n";
        ...
        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            ...
            vk_math::Mat4 view = vk_math::Mat4::lookAt(vk_math::Vec3(0.0f, 1.3f, 1.8f), vk_math::Vec3(0.0f, 0.0f, 0.0f), vk_math::Vec3(0.0f, 1.0f, 0.0f));
            ...
            pc.model = model;
            pc.simParams = Vec4(time, 8.0f, 0.15f, 0.0f);
        ...
        std::cout << "\nAssignment 88 executed cleanly (400 frames rendered).\n";
    }
