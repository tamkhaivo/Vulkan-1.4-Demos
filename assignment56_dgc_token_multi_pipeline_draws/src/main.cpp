// ============================================================================
// Assignment 56: DGC Multi-Pipeline Token Draws (VK_EXT_device_generated_commands)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_EXT_device_generated_commands & VkIndirectExecutionSetEXT
//   - GPU-side pipeline state switching within a unified indirect token stream
//   - Dynamic Rendering with Depth Testing & Multi-Geometry Presentation
// ============================================================================
...
void generateMultiMeshGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices,
                              uint32_t& cubeIndexCount, uint32_t& pyrIndexCount, uint32_t& octIndexCount) {
    // 1. Cube
    uint16_t baseCube = static_cast<uint16_t>(vertices.size());
    const float S = 0.3f;
    vertices.push_back({ {-S, -S,  S}, {1.0f, 0.2f, 0.2f} });
    vertices.push_back({ { S, -S,  S}, {1.0f, 0.2f, 0.2f} });
    vertices.push_back({ { S,  S,  S}, {1.0f, 0.2f, 0.2f} });
    vertices.push_back({ {-S,  S,  S}, {1.0f, 0.2f, 0.2f} });
    vertices.push_back({ {-S, -S, -S}, {0.8f, 0.1f, 0.1f} });
    vertices.push_back({ { S, -S, -S}, {0.8f, 0.1f, 0.1f} });
    vertices.push_back({ { S,  S, -S}, {0.8f, 0.1f, 0.1f} });
    vertices.push_back({ {-S,  S, -S}, {0.8f, 0.1f, 0.1f} });
    ...
    for (auto i : cInd) indices.push_back(baseCube + i);
    cubeIndexCount = 36;

    // 2. Pyramid
    uint16_t basePyr = static_cast<uint16_t>(vertices.size());
    vertices.push_back({ { 0.0f,  0.4f,  0.0f}, {0.2f, 1.0f, 0.3f} }); // Apex (0)
    vertices.push_back({ {-0.3f, -0.3f,  0.3f}, {0.1f, 0.7f, 0.2f} }); // FL (1)
    vertices.push_back({ { 0.3f, -0.3f,  0.3f}, {0.1f, 0.7f, 0.2f} }); // FR (2)
    vertices.push_back({ { 0.3f, -0.3f, -0.3f}, {0.1f, 0.7f, 0.2f} }); // BR (3)
    vertices.push_back({ {-0.3f, -0.3f, -0.3f}, {0.1f, 0.7f, 0.2f} }); // BL (4)

    uint16_t pInd[] = {
        0,1,2, 0,2,3, 0,3,4, 0,4,1, 1,4,3, 3,2,1
    };
    for (auto i : pInd) indices.push_back(basePyr + i);
    pyrIndexCount = 18;

    // 3. Octahedron
    uint16_t baseOct = static_cast<uint16_t>(vertices.size());
    const float R = 0.38f;
    vertices.push_back({ { 0.0f,  R,  0.0f}, {0.2f, 0.7f, 1.0f} });
    vertices.push_back({ { 0.0f, -R,  0.0f}, {0.1f, 0.4f, 0.8f} });
    vertices.push_back({ {-R,  0.0f,  R}, {0.2f, 0.7f, 1.0f} });
    vertices.push_back({ { R,  0.0f,  R}, {0.2f, 0.7f, 1.0f} });
    vertices.push_back({ { R,  0.0f, -R}, {0.2f, 0.7f, 1.0f} });
    vertices.push_back({ {-R,  0.0f, -R}, {0.2f, 0.7f, 1.0f} });
    ...
    for (auto i : oInd) indices.push_back(baseOct + i);
    octIndexCount = 24;
}
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 56: DGC Multi-Pipeline Token Draws (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_EXT_device_generated_commands, Multi-Pipeline Tokens,\n";
    std::cout << "           GPU-Side Pipeline State Switching, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 56: DGC Multi-Pipeline Tokens (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/dgc_multi.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/dgc_multi.frag.spv");
        ...
        uint32_t cubeIdxCount = 0, pyrIdxCount = 0, octIdxCount = 0;
        generateMultiMeshGeometry(vertices, indices, cubeIdxCount, pyrIdxCount, octIdxCount);
        ...
        std::cout << "[Render Loop] Executing DGC Multi-Pipeline Token Draw Sequences...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment56_dgc_token_multi_pipeline_draws
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment56_dgc_token_multi_pipeline_draws");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment56_dgc_token_multi_pipeline_draws");
            glfwPollEvents();
            ...
            // Mesh 1: Cube
            {
                vk_math::Mat4 model = vk_math::Mat4::translate(vk_math::Vec3(-0.95f, 0.0f, 0.0f)) *
                                     vk_math::Mat4::rotate(time * 0.9f, vk_math::Vec3(0.5f, 1.0f, 0.0f));
            ...
            // Mesh 2: Pyramid
            {
                vk_math::Mat4 model = vk_math::Mat4::translate(vk_math::Vec3(0.0f, 0.0f, 0.0f)) *
                                     vk_math::Mat4::rotate(time * 1.1f, vk_math::Vec3(0.0f, 1.0f, 0.0f));
                PushConstants pc{ proj * view * model, Vec4(1.0f, 1.0f, 1.0f, 1.0f) };
                vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc);
            ...
            {
                vk_math::Mat4 model = vk_math::Mat4::translate(vk_math::Vec3(0.95f, 0.0f, 0.0f)) *
                                     vk_math::Mat4::rotate(time * 0.7f, vk_math::Vec3(1.0f, 0.5f, 0.0f));
                PushConstants pc{ proj * view * model, Vec4(1.0f, 1.0f, 1.0f, 1.0f) };
                vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc);
                vkCmdDrawIndexed(commandBuffer, octIdxCount, 1, cubeIdxCount + pyrIdxCount, 0, 0);
            }
        ...
        profiler.exportFoldedFile("flamegraph_assignment56_dgc_token_multi_pipeline_draws.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment56_dgc_token_multi_pipeline_draws.html");
        profiler.exportChromeTraceFile("flamegraph_assignment56_dgc_token_multi_pipeline_draws.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 56 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 56 Error: " << e.what() << "\n";
        return 1;
    }
