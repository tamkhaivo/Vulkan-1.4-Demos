...
//   - Compute Ray Budgeting & Dynamic Resolution Ray Grid Dispatches
...
struct PushConstants {
    vk_math::Mat4 mvp;
    Vec4 dispatchParams; // x: budget ratio, yzw: color tint
};

struct VkTraceRaysIndirectCommandKHR_Local {
    uint32_t width;
    uint32_t height;
    uint32_t depth;
};

// Generate 3D Icosahedron Geometry
void generateIcosahedronGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    std::vector<vk_math::Vec3> v = {
        {-s,  t * s, 0.0f}, { s,  t * s, 0.0f}, {-s, -t * s, 0.0f}, { s, -t * s, 0.0f},
        {0.0f, -s,  t * s}, {0.0f,  s,  t * s}, {0.0f, -s, -t * s}, {0.0f,  s, -t * s},
        { t * s, 0.0f, -s}, { t * s, 0.0f,  s}, {-t * s, 0.0f, -s}, {-t * s, 0.0f,  s}
    };

    for (const auto& p : v) {
        float r = 0.5f + 0.5f * p.x;
        float g = 0.5f + 0.5f * p.y;
        float b = 0.5f + 0.5f * p.z;
        vertices.push_back({ p, {r, g, b} });
    }
...
int main() {
    ...
    std::cout << " Concepts: vkCmdTraceRaysIndirectKHR, VkTraceRaysIndirectCommandKHR,\n";
    std::cout << "           Compute Ray Budgeting, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 60: Indirect Ray Tracing Dispatch (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/indirect_vis.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/indirect_vis.frag.spv");
        ...
        // Indirect Command Buffer (VkTraceRaysIndirectCommandKHR)
        VkTraceRaysIndirectCommandKHR_Local indirectCmd{ 800, 600, 1 };
        VkBuffer indirectBuffer;
        VkDeviceMemory indirectBufferMemory;
        createBuffer(device, physicalDevice, sizeof(VkTraceRaysIndirectCommandKHR_Local),
                     VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     indirectBuffer, indirectBufferMemory);
        void* cmdData;
        vkMapMemory(device, indirectBufferMemory, 0, sizeof(VkTraceRaysIndirectCommandKHR_Local), 0, &cmdData);
        std::memcpy(cmdData, &indirectCmd, sizeof(VkTraceRaysIndirectCommandKHR_Local));
        ...
        std::cout << "[Render Loop] Rendering with Indirect Ray Tracing Dispatch (vkCmdTraceRaysIndirectKHR)...\n";
        ...
        std::cout << "\nAssignment 60 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 60 Error: " << e.what() << "\n";
        return 1;
    }
