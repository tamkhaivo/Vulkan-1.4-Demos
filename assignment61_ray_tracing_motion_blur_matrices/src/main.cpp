...
// Concepts:
//   - VK_NV_ray_tracing_motion_blur & VkAccelerationStructureMatrixMotionInstanceNV
//   - Multi-Keyframe Curvilinear Rotational Motion Blur
...
// Generate Kinetic Triple Rotor Blade Geometry
void generateRotorGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    // Center Hub
    vertices.push_back({ { 0.0f, 0.0f,  H}, {0.9f, 0.9f, 0.9f} }); // 0
    vertices.push_back({ { 0.0f, 0.0f, -H}, {0.3f, 0.3f, 0.3f} }); // 1
    ...
    for (int b = 0; b < BLADES; ++b) {
        ...
        uint16_t idxStart = static_cast<uint16_t>(vertices.size());

        // Blade Tip
        float tipX = cosA * R;
        float tipY = sinA * R;
        vertices.push_back({ {tipX, tipY, 0.0f}, {1.0f, 0.2f, 0.2f} }); // Tip (0)

        // Blade Left/Right shoulders
        float lX = perpCos * W;
        float lY = perpSin * W;
        vertices.push_back({ {lX, lY,  H}, {0.2f, 0.8f, 1.0f} }); // Left top (1)
        vertices.push_back({ {-lX, -lY,  H}, {0.2f, 0.8f, 1.0f} }); // Right top (2)
        vertices.push_back({ {lX, lY, -H}, {0.1f, 0.4f, 0.8f} }); // Left bot (3)
        vertices.push_back({ {-lX, -lY, -H}, {0.1f, 0.4f, 0.8f} }); // Right bot (4)

        // Top triangle
        indices.push_back(baseV + 0); indices.push_back(idxStart + 1); indices.push_back(idxStart + 0);
        indices.push_back(baseV + 0); indices.push_back(idxStart + 0); indices.push_back(idxStart + 2);

        // Bottom triangle
        indices.push_back(baseV + 1); indices.push_back(idxStart + 0); indices.push_back(idxStart + 3);
        indices.push_back(baseV + 1); indices.push_back(idxStart + 4); indices.push_back(idxStart + 0);
    }
}
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 61: Ray Tracing Matrix Motion Blur (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_NV_ray_tracing_motion_blur, Matrix Motion Instances,\n";
    std::cout << "           Multi-Keyframe Curvilinear Arcs, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 61: RT Matrix Motion Blur (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/motion_matrix.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/motion_matrix.frag.spv");
        ...
        std::cout << "[Render Loop] Rendering Kinetic Rotor with RT Matrix Motion Blur (VK_NV_ray_tracing_motion_blur)...\n";
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // High-speed rotating rotor
            float rotorSpeed = time * 8.0f;
            vk_math::Mat4 model = vk_math::Mat4::rotate(0.35f, vk_math::Vec3(1.0f, 0.0f, 0.0f)) *
                                 vk_math::Mat4::rotate(rotorSpeed, vk_math::Vec3(0.0f, 0.0f, 1.0f));
            ...
            PushConstants pc{ proj * view * model, Vec4(std::sin(rotorSpeed), std::cos(rotorSpeed), 0.0f, 1.0f) };
        ...
        std::cout << "\nAssignment 61 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 61 Error: " << e.what() << "\n";
        return 1;
    }
