...
// Concepts:
//   - VK_KHR_calibrated_timestamps & vkGetCalibratedTimestampsEXT
//   - Simultaneous CPU (QPC/Monotonic) & GPU Device Clock Domain Calibration
//   - VkQueryPool (VK_QUERY_TYPE_TIMESTAMP) & vkCmdWriteTimestamp2
//   - Nanosecond-precision frame telemetry and Dynamic Rendering
// ============================================================================
...
int main() {
    ...
    std::cout << " Concepts: VK_KHR_calibrated_timestamps, vkCmdWriteTimestamp2,\n";
    std::cout << "           Nanosecond GPU Telemetry, Simultaneous Host/Device Clocks\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 28: Calibrated Timestamps Profiling (Vulkan 1.4)");
        ...
        float timestampPeriod = deviceProps.limits.timestampPeriod; // ns per tick
        std::cout << "GPU Device: " << deviceProps.deviceName << " (Timestamp Period: " << timestampPeriod << " ns/tick)\n";
        ...
        auto pfn_vkGetCalibratedTimestampsEXT = (PFN_vkGetCalibratedTimestampsEXT)vkGetDeviceProcAddr(device, "vkGetCalibratedTimestampsEXT");
        ...
        // Timestamp Query Pool (2 queries: Pass Start & Pass End)
        VkQueryPoolCreateInfo queryPoolInfo{};
        queryPoolInfo.sType = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO;
        queryPoolInfo.queryType = VK_QUERY_TYPE_TIMESTAMP;
        queryPoolInfo.queryCount = 2;
        VkQueryPool queryPool;
        vk_common::check_vk_result(vkCreateQueryPool(device, &queryPoolInfo, nullptr, &queryPool), "Failed to create query pool");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/profiled_scene.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/profiled_scene.frag.spv");
        ...
        std::cout << "[Render Loop] Profiling render pass execution using Calibrated Timestamps...\n";
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // Read back Query Pool Results from the previous completed frame
            if (frameCount > 0) {
                uint64_t queryResults[2] = { 0, 0 };
                VkResult qRes = vkGetQueryPoolResults(
                    device, queryPool, 0, 2, sizeof(queryResults), queryResults,
                    sizeof(uint64_t), VK_QUERY_RESULT_64_BIT
                );
                if (qRes == VK_SUCCESS && queryResults[1] >= queryResults[0]) {
                    uint64_t gpuTicks = queryResults[1] - queryResults[0];
                    double gpuDurationUs = (double)gpuTicks * (double)timestampPeriod / 1000.0;

                    if (frameCount % 120 == 0) {
                        std::cout << std::fixed << std::setprecision(2)
                                  << "[Telemetry Frame " << frameCount << "] Render Pass GPU Duration: "
                                  << gpuDurationUs << " µs (" << gpuTicks << " ticks)\n";
                    }
                }
            }
            ...
            // TIMESTAMP 0: Start of Render Pass
            ...
            colorAttachment.clearValue = { { { 0.05f, 0.05f, 0.10f, 1.0f } } };
            ...
            // TIMESTAMP 1: End of Render Pass
        ...
        std::cout << "\nAssignment 28 executed cleanly (" << frameCount << " profiled frames rendered).\n";
