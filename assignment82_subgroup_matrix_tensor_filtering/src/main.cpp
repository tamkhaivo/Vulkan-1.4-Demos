...
// Concepts:
//   - VK_KHR_cooperative_matrix & Wave-Level Matrix Tiling
//   - High-Throughput FP16 Matrix Multiply-Accumulate Filtering
//   - Dynamic Rendering with Split-Screen Visual Feedback
// ============================================================================
...
void generateDisplayGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    // Front Screen
    vertices.push_back({ vk_math::Vec3(-W, -H,  D), {0.0f, 1.0f} });
    vertices.push_back({ vk_math::Vec3( W, -H,  D), {1.0f, 1.0f} });
    vertices.push_back({ vk_math::Vec3( W,  H,  D), {1.0f, 0.0f} });
    vertices.push_back({ vk_math::Vec3(-W,  H,  D), {0.0f, 0.0f} });
    ...
    for (auto i : boxInd) indices.push_back(baseV + i);
}
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 82: Subgroup Matrix Tensor Neural Filtering (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_KHR_cooperative_matrix, 16x16 Wave Matrix Tiling,\n";
    std::cout << "           Real-Time Neural Denoising Filter & Dynamic Rendering\n";
    std::cout << "====================================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 82: Cooperative Matrix Neural Filter (Vulkan 1.4)");
        ...
        generateDisplayGeometry(vertices, indices);
        ...
        std::string shaderDir = "assignment82_subgroup_matrix_tensor_filtering/shaders/";
        if (!std::filesystem::exists(shaderDir + "tensor_filter.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "tensor_filter.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "tensor_filter.frag.spv");
        ...
        std::cout << "[Cooperative Matrix] Tensor Core 16x16 Wave Tile Kernel initialized.\n";
        std::cout << "[Render Loop] Executing Subgroup Matrix Denoising & Split Comparison Loop...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        int frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment82_subgroup_matrix_tensor_filtering
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment82_subgroup_matrix_tensor_filtering");
        profiler.initGpu(device, physicalDevice);

        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            VK_PROFILE_SCOPE("assignment82_subgroup_matrix_tensor_filtering");
            glfwPollEvents();
            ...
            PushConstants pc{};
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.3f, vk_math::Vec3(0.0f, 1.0f, 0.0f));
            ...
            pc.filterParams = Vec4(time, 0.0f, 0.0f, 0.0f);
        ...
        profiler.exportFoldedFile("flamegraph_assignment82_subgroup_matrix_tensor_filtering.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment82_subgroup_matrix_tensor_filtering.html");
        profiler.exportChromeTraceFile("flamegraph_assignment82_subgroup_matrix_tensor_filtering.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 82 executed cleanly (400 frames rendered).\n";
    }
