// ============================================================================
// Assignment 27: Extended Dynamic State 3 & Dynamic Pipelines
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_EXT_extended_dynamic_state3 & VK_EXT_extended_dynamic_state
//   - Zero-PSO-explosion: Single unified pipeline for solid, wireframe, and blended
//   - Dynamic Polygon Mode, Dynamic Blend Equations, Dynamic Cull Mode
...
// Generate an Icosahedron / Geosphere mesh
...
int main() {
    ...
    std::cout << " Concepts: Single Pipeline with Dynamic Polygon Mode, Dynamic Blend,\n";
    std::cout << "           Dynamic Cull Mode, Dynamic Rendering, Zero PSO Overhead\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 27: Extended Dynamic State 3 (Vulkan 1.4)");
        ...
        edsFeatures.pNext = &eds3Features;
        edsFeatures.extendedDynamicState = VK_TRUE;

        VkPhysicalDeviceVulkan13Features vulkan13Features{};
        vulkan13Features.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES;
        vulkan13Features.pNext = &edsFeatures;
        ...
        auto pfn_vkCmdSetPolygonModeEXT = (PFN_vkCmdSetPolygonModeEXT)vkGetDeviceProcAddr(device, "vkCmdSetPolygonModeEXT");
        ...
        // Load Shaders
        auto vertCode = vulkan_utils::readFile("shaders/dynamic_state.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/dynamic_state.frag.spv");
        ...
        vk_common::check_vk_result(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &graphicsCreateInfo, nullptr, &graphicsPipeline), "Failed to create dynamic pipeline");
        ...
        std::cout << "[Render Loop] Rendering 3 spheres using 1 single PSO with dynamically swapped polygon modes & culling...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment27_extended_dynamic_state3
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment27_extended_dynamic_state3");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment27_extended_dynamic_state3");
            glfwPollEvents();
            ...
            // --- DRAW 1: Left Sphere - SOLID FILL (Dynamic State 1) ---
            if (pfn_vkCmdSetPolygonModeEXT) {
                pfn_vkCmdSetPolygonModeEXT(commandBuffer, VK_POLYGON_MODE_FILL);
            }
            vkCmdSetCullMode(commandBuffer, VK_CULL_MODE_BACK_BIT);

            vk_math::Mat4 model1 = vk_math::Mat4::translate(vk_math::Vec3(-1.3f, 0.0f, 0.0f)) *
                                  vk_math::Mat4::rotate(time * 0.6f, vk_math::Vec3(0.3f, 1.0f, 0.0f));
            PushConstants pc1{ proj * view * model1, Vec4(1.0f, 0.8f, 0.2f, 1.0f) };
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc1);
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);

            // --- DRAW 2: Center Sphere - DYNAMIC WIREFRAME (Dynamic State 2 - Same Pipeline!) ---
            if (pfn_vkCmdSetPolygonModeEXT) {
                pfn_vkCmdSetPolygonModeEXT(commandBuffer, VK_POLYGON_MODE_LINE);
            }
            vkCmdSetCullMode(commandBuffer, VK_CULL_MODE_NONE);

            vk_math::Mat4 model2 = vk_math::Mat4::translate(vk_math::Vec3(0.0f, 0.0f, 0.0f)) *
                                  vk_math::Mat4::rotate(time * 0.9f, vk_math::Vec3(1.0f, 0.2f, 0.5f));
            PushConstants pc2{ proj * view * model2, Vec4(0.2f, 0.9f, 1.0f, 1.0f) };
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc2);
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);

            // --- DRAW 3: Right Sphere - DYNAMIC POINT / ROTATED (Dynamic State 3 - Same Pipeline!) ---
            if (pfn_vkCmdSetPolygonModeEXT) {
                pfn_vkCmdSetPolygonModeEXT(commandBuffer, VK_POLYGON_MODE_POINT);
            }
            vkCmdSetCullMode(commandBuffer, VK_CULL_MODE_NONE);

            vk_math::Mat4 model3 = vk_math::Mat4::translate(vk_math::Vec3(1.3f, 0.0f, 0.0f)) *
                                  vk_math::Mat4::rotate(time * 1.2f, vk_math::Vec3(0.0f, 1.0f, 0.8f));
            PushConstants pc3{ proj * view * model3, Vec4(1.0f, 0.3f, 0.7f, 1.0f) };
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc3);
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), 1, 0, 0, 0);
        ...
        profiler.exportFoldedFile("flamegraph_assignment27_extended_dynamic_state3.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment27_extended_dynamic_state3.html");
        profiler.exportChromeTraceFile("flamegraph_assignment27_extended_dynamic_state3.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 27 executed cleanly (" << frameCount << " frames rendered).\n";
