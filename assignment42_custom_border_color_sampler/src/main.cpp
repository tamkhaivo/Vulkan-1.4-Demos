...
// Concepts:
//   - VK_EXT_custom_border_color & VK_BORDER_COLOR_FLOAT_CUSTOM_EXT
//   - VkSamplerCustomBorderColorCreateInfoEXT with arbitrary RGBA color
//   - Dynamic UV scaling triggering custom border color clamping in real time
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 42: Custom Border Color Sampler (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_EXT_custom_border_color, VK_BORDER_COLOR_FLOAT_CUSTOM_EXT,\n";
    std::cout << "           VkSamplerCustomBorderColorCreateInfoEXT, Dynamic UV Clamping\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 42: Custom Border Color (Vulkan 1.4)");
        ...
        vulkan13Features.pNext = &customBorderFeatures;
        ...
        // Texture Image Creation (128x128 Procedural Tile)
        ...
        for (uint32_t y = 0; y < TEX_H; ++y) {
            for (uint32_t x = 0; x < TEX_W; ++x) {
                bool checker = ((x / 16) % 2) == ((y / 16) % 2);
                texPixels[y * TEX_W + x] = checker ? 0xFFFFFFFF : 0xFF202020;
            }
        }
        ...
        vk_common::check_vk_result(vkAllocateMemory(device, &texAlloc, nullptr, &textureImageMemory), "Failed to allocate tex mem");
        ...
        // Staging buffer to populate texture
        ...
        VkCommandPoolCreateInfo initPoolInfo{ VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO };
        initPoolInfo.queueFamilyIndex = graphicsQueueFamily;
        VkCommandPool initPool;
        vkCreateCommandPool(device, &initPoolInfo, nullptr, &initPool);

        VkCommandBufferAllocateInfo initCmdAlloc{ VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO, nullptr, initPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1 };
        VkCommandBuffer initCmd;
        vkAllocateCommandBuffers(device, &initCmdAlloc, &initCmd);

        VkCommandBufferBeginInfo initBegin{ VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO, nullptr, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT };
        ...
        toDstBarrier.image = textureImage;
        toDstBarrier.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 };

        VkDependencyInfo depToDst{ VK_STRUCTURE_TYPE_DEPENDENCY_INFO, nullptr, 0, 0, nullptr, 0, nullptr, 1, &toDstBarrier };
        vk14.vkCmdPipelineBarrier2(initCmd, &depToDst);

        VkBufferImageCopy copyRegion{};
        copyRegion.imageSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 };
        copyRegion.imageExtent = { TEX_W, TEX_H, 1 };
        vkCmdCopyBufferToImage(initCmd, stagingBuffer, textureImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion);

        VkImageMemoryBarrier2 toReadBarrier{};
        toReadBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2;
        toReadBarrier.srcStageMask = VK_PIPELINE_STAGE_2_TRANSFER_BIT;
        toReadBarrier.srcAccessMask = VK_ACCESS_2_TRANSFER_WRITE_BIT;
        ...
        toReadBarrier.image = textureImage;
        toReadBarrier.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 };

        VkDependencyInfo depToRead{ VK_STRUCTURE_TYPE_DEPENDENCY_INFO, nullptr, 0, 0, nullptr, 0, nullptr, 1, &toReadBarrier };
        vk14.vkCmdPipelineBarrier2(initCmd, &depToRead);

        vkEndCommandBuffer(initCmd);

        VkSubmitInfo initSubmit{ VK_STRUCTURE_TYPE_SUBMIT_INFO, nullptr, 0, nullptr, nullptr, 1, &initCmd };
        vkQueueSubmit(graphicsQueue, 1, &initSubmit, VK_NULL_HANDLE);
        ...
        // CREATE CUSTOM BORDER COLOR SAMPLER (VK_BORDER_COLOR_FLOAT_CUSTOM_EXT)
        // Custom vibrant Neon Cyan border color (0.0, 1.0, 0.9, 1.0)
        ...
        customBorderInfo.customBorderColor.float32[0] = 0.0f; // Red
        customBorderInfo.customBorderColor.float32[1] = 1.0f; // Green
        customBorderInfo.customBorderColor.float32[2] = 0.85f; // Blue
        customBorderInfo.customBorderColor.float32[3] = 1.0f; // Alpha
        customBorderInfo.format = VK_FORMAT_UNDEFINED;

        VkSamplerCreateInfo samplerInfo{};
        samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
        samplerInfo.pNext = &customBorderInfo;
        ...
        VkSampler customSampler;
        if (vkCreateSampler(device, &samplerInfo, nullptr, &customSampler) != VK_SUCCESS) {
            // Fallback to standard clamp to border if custom border extension isn't hardware supported
        ...
        // Load Shaders
        auto vertCode = vulkan_utils::readFile("shaders/custom_border.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/custom_border.frag.spv");
        ...
        VkPipelineRenderingCreateInfo renderingCreateInfo{ VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO, nullptr, 0, 1, &surfaceFormat.format, VK_FORMAT_UNDEFINED, VK_FORMAT_UNDEFINED };

        VkGraphicsPipelineCreateInfo graphicsCreateInfo{
            VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO, &renderingCreateInfo, 0,
            2, shaderStages, &vertexInputInfo, &inputAssembly, nullptr, &viewportState, &rasterizer, &multisampling, nullptr, &colorBlending, &dynamicState,
        ...
        VkDescriptorImageInfo descImgInfo{ customSampler, textureImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
        ...
        std::cout << "[Render Loop] Rendering textured quad with vibrant Neon Cyan Custom Border Color Clamping...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment42_custom_border_color_sampler
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment42_custom_border_color_sampler");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment42_custom_border_color_sampler");
            glfwPollEvents();
            ...
            // Modulate UV scale between 0.6x and 2.2x to dynamically showcase custom border clamping
            float uvScale = 1.4f + 0.8f * std::sin(time * 1.5f);
            ...
            VkDependencyInfo depToColor{ VK_STRUCTURE_TYPE_DEPENDENCY_INFO, nullptr, 0, 0, nullptr, 0, nullptr, 1, &barrierToColor };
            ...
            colorAttachment.clearValue = { { { 0.04f, 0.04f, 0.07f, 1.0f } } };
            ...
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.4f, vk_math::Vec3(0.0f, 0.0f, 1.0f));
            vk_math::Mat4 view = vk_math::Mat4::lookAt(vk_math::Vec3(0.0f, 0.0f, 2.0f), vk_math::Vec3(0.0f, 0.0f, 0.0f), vk_math::Vec3(0.0f, 1.0f, 0.0f));
            vk_math::Mat4 proj = vk_math::Mat4::perspective(vk_math::radians(45.0f), (float)WIDTH / (float)HEIGHT, 0.1f, 100.0f);

            PushConstants pc{ proj * view * model, uvScale };
            ...
            VkDependencyInfo depToPresent{ VK_STRUCTURE_TYPE_DEPENDENCY_INFO, nullptr, 0, 0, nullptr, 0, nullptr, 1, &barrierToPresent };
            vk14.vkCmdPipelineBarrier2(commandBuffer, &depToPresent);

            vkEndCommandBuffer(commandBuffer);

            VkPipelineStageFlags waitStages[] = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };
            VkSubmitInfo submitInfo{ VK_STRUCTURE_TYPE_SUBMIT_INFO, nullptr, 1, &imageAvailableSemaphore, waitStages, 1, &commandBuffer, 1, &renderFinishedSemaphore };

            vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFence);

            VkPresentInfoKHR presentInfo{ VK_STRUCTURE_TYPE_PRESENT_INFO_KHR, nullptr, 1, &renderFinishedSemaphore, 1, &swapchain, &imageIndex, nullptr };
        ...
        profiler.exportFoldedFile("flamegraph_assignment42_custom_border_color_sampler.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment42_custom_border_color_sampler.html");
        profiler.exportChromeTraceFile("flamegraph_assignment42_custom_border_color_sampler.json");
        profiler.cleanupGpu();
        ...
        vkDestroySampler(device, customSampler, nullptr);
        ...
        std::cout << "\nAssignment 42 executed cleanly (" << frameCount << " frames rendered).\n";
