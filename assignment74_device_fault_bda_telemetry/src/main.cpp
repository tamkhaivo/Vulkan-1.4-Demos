// ============================================================================
// Assignment 74: Device Address Binding Reporting & GPU Telemetry (VK_EXT_device_address_binding_report)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_EXT_device_address_binding_report & VK_EXT_device_fault
//   - Raw 64-bit Buffer Device Address (BDA) pointer rendering
//   - Dynamic Rendering with Depth Testing & Address Range Registry
// ============================================================================
...
// Global CPU BDA Address Tracker Map
struct GpuBufferMemoryRange {
    VkDeviceAddress startAddress;
    VkDeviceSize size;
    std::string name;
};
static std::map<VkDeviceAddress, GpuBufferMemoryRange> g_BdaRegistry;

void registerGpuBufferAddress(VkDeviceAddress addr, VkDeviceSize size, const std::string& name) {
    g_BdaRegistry[addr] = { addr, size, name };
    std::cout << "[BDA Registry] Registered " << name << " at 0x" << std::hex << addr << " (Size: " << std::dec << size << " bytes)\n";
}

// Generate 3D Octahedron Geometry
void generateOctahedronGeometry(std::vector<Vertex>& vertices) {
    // 8 triangles of octahedron (24 vertices)
    vk_math::Vec3 top(0.0f, 0.8f, 0.0f);
    vk_math::Vec3 bottom(0.0f, -0.8f, 0.0f);
    vk_math::Vec3 p1(0.7f, 0.0f, 0.0f);
    vk_math::Vec3 p2(0.0f, 0.0f, 0.7f);
    vk_math::Vec3 p3(-0.7f, 0.0f, 0.0f);
    vk_math::Vec3 p4(0.0f, 0.0f, -0.7f);

    auto addTri = [&](vk_math::Vec3 a, vk_math::Vec3 b, vk_math::Vec3 c, vk_math::Vec3 col) {
        vertices.push_back({ a, 0.0f, col, 0.0f });
        vertices.push_back({ b, 0.0f, col, 0.0f });
        vertices.push_back({ c, 0.0f, col, 0.0f });
    };

    // Top 4 faces
    addTri(top, p1, p2, { 1.0f, 0.2f, 0.3f });
    addTri(top, p2, p3, { 0.2f, 1.0f, 0.4f });
    addTri(top, p3, p4, { 0.3f, 0.4f, 1.0f });
    addTri(top, p4, p1, { 1.0f, 0.9f, 0.2f });

    // Bottom 4 faces
    addTri(bottom, p2, p1, { 0.9f, 0.3f, 0.8f });
    addTri(bottom, p3, p2, { 0.2f, 0.8f, 0.9f });
    addTri(bottom, p4, p3, { 0.8f, 0.5f, 0.2f });
    addTri(bottom, p1, p4, { 0.5f, 0.9f, 0.3f });
}
...
int main() {
    std::cout << "====================================================================\n";
    std::cout << " Assignment 74: Device Address Binding Telemetry (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_EXT_device_address_binding_report, Raw 64-bit BDA,\n";
    std::cout << "           GPU Virtual Address Isolation & Crash Fault Telemetry\n";
    std::cout << "====================================================================\n";
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Assignment 74: Device Address Binding Telemetry", nullptr, nullptr);
    if (!window) {
    ...
    try {
        ...
        appInfo.pApplicationName = "Assignment 74 - BDA Telemetry";
        ...
        // Configure BDA, Dynamic Rendering, Synchronization2
        ...
        // 64-Bit BDA Vertex Buffer
        std::vector<Vertex> vertices;
        generateOctahedronGeometry(vertices);
        ...
        // Query 64-Bit GPU Address
        VkBufferDeviceAddressInfo bdaInfo{ VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO };
        bdaInfo.buffer = vertexBuffer;
        VkDeviceAddress vertexBufferGpuAddress = vk14.vkGetBufferDeviceAddress(device, &bdaInfo);

        // Register in CPU telemetry registry
        registerGpuBufferAddress(vertexBufferGpuAddress, vertexBufferSize, "Octahedron_VertexBuffer_BDA");
        ...
        std::string shaderDir = "assignment74_device_fault_bda_telemetry/shaders/";
        if (!std::filesystem::exists(shaderDir + "bda_fault.vert.spv")) shaderDir = "shaders/";

        auto vertCode = vulkan_utils::readFile(shaderDir + "bda_fault.vert.spv");
        auto fragCode = vulkan_utils::readFile(shaderDir + "bda_fault.frag.spv");
        ...
        // Descriptorless BDA Pipeline: No vertex buffer bindings needed!
        ...
        std::cout << "[Render Loop] Executing Raw 64-Bit BDA Direct Pointer Rendering Loop...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment74_device_fault_bda_telemetry
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment74_device_fault_bda_telemetry");
        profiler.initGpu(device, physicalDevice);

        while (!glfwWindowShouldClose(window) && frameCount < 400) {
            VK_PROFILE_SCOPE("assignment74_device_fault_bda_telemetry");
            glfwPollEvents();
            ...
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 1.5f, { 0.0f, 1.0f, 0.0f }) * vk_math::Mat4::rotate(time * 0.7f, { 0.0f, 0.0f, 1.0f });
            ...
            pc.vertexBufferAddress = vertexBufferGpuAddress; // Inject raw 64-bit device memory pointer directly!
            ...
            // Directly draw 24 vertices from BDA pointer
        ...
        profiler.exportFoldedFile("flamegraph_assignment74_device_fault_bda_telemetry.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment74_device_fault_bda_telemetry.html");
        profiler.exportChromeTraceFile("flamegraph_assignment74_device_fault_bda_telemetry.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 74 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 74 Error: " << e.what() << "\n";
        return 1;
    }
