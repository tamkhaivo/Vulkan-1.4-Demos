// ============================================================================
// Assignment 69: Acceleration Structure Compaction & Serialization (VK_KHR_acceleration_structure)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - VK_KHR_acceleration_structure & VK_QUERY_TYPE_ACCELERATION_STRUCTURE_COMPACTED_SIZE_KHR
//   - GPU-Side BVH Compaction & Disk Serialization Pipeline
//   - Dynamic Rendering with Depth Testing & Compacted Geometric Mesh
// ============================================================================
...
// Generate 3D Dodecahedron BVH Cluster Geometry with exact regular dodecahedron vertices and planar centroid fans
void generateDodecahedronGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    const float phi = (1.0f + std::sqrt(5.0f)) * 0.5f;
    const float invPhi = 1.0f / phi;
    const float scale = 0.55f;

    // Standard Cartesian coordinates for regular dodecahedron vertices (scaled):
    // (±1, ±1, ±1)
    // (0, ±1/phi, ±phi)
    // (±1/phi, ±phi, 0)
    // (±phi, 0, ±1/phi)
    std::vector<vk_math::Vec3> rawPts = {
        // 0..7: (±1, ±1, ±1)
        {-1.0f * scale, -1.0f * scale, -1.0f * scale}, // 0: ---
        {-1.0f * scale, -1.0f * scale,  1.0f * scale}, // 1: --+
        {-1.0f * scale,  1.0f * scale, -1.0f * scale}, // 2: -+-
        {-1.0f * scale,  1.0f * scale,  1.0f * scale}, // 3: -++
        { 1.0f * scale, -1.0f * scale, -1.0f * scale}, // 4: +--
        { 1.0f * scale, -1.0f * scale,  1.0f * scale}, // 5: +-++
        { 1.0f * scale,  1.0f * scale, -1.0f * scale}, // 6: ++-
        { 1.0f * scale,  1.0f * scale,  1.0f * scale}, // 7: +++
        // 8..11: (0, ±1/phi, ±phi)
        { 0.0f, -invPhi * scale, -phi * scale}, // 8
        { 0.0f, -invPhi * scale,  phi * scale}, // 9
        { 0.0f,  invPhi * scale, -phi * scale}, // 10
        { 0.0f,  invPhi * scale,  phi * scale}, // 11
        // 12..15: (±1/phi, ±phi, 0)
        {-invPhi * scale, -phi * scale,  0.0f}, // 12
        {-invPhi * scale,  phi * scale,  0.0f}, // 13
        { invPhi * scale, -phi * scale,  0.0f}, // 14
        { invPhi * scale,  phi * scale,  0.0f}, // 15
        // 16..19: (±phi, 0, ±1/phi)
        {-phi * scale,  0.0f, -invPhi * scale}, // 16
        { phi * scale,  0.0f, -invPhi * scale}, // 17
        {-phi * scale,  0.0f,  invPhi * scale}, // 18
        { phi * scale,  0.0f,  invPhi * scale}  // 19
    };

    // Correct adjacent 5-cycle indices for each of the 12 pentagons of the standard regular dodecahedron
    int faces[12][5] = {
        { 3, 11,  7, 15, 13}, // Top-ish (+Y)
        { 7, 19, 17,  6, 15}, // +X +Y
        { 7, 11,  9,  5, 19}, // +Z +X
        {11,  3,  1,  9, 18}, // +Z -X (order: 11, 3, 18, 1, 9 corrected below)
        { 3, 13,  2, 16, 18}, // -X +Y
        { 2, 10,  6, 15, 13}, // -Z +Y
        { 6, 17,  4,  8, 10}, // +X -Z
        { 2, 10,  8,  0, 16}, // -X -Z
        ...
        { 1,  9,  5, 14, 12}, // -Y +Z
        { 5, 19, 17,  4, 14}, // +X -Y
        { 4, 14, 12,  0,  8}  // Bottom-ish (-Y)
    };

    // Exact sorted 5-cycles ensuring perimeter connectivity:
    int validFaces[12][5] = {
        { 3, 11,  7, 15, 13},
        { 7, 19, 17,  6, 15},
        { 7, 11,  9,  5, 19},
        ...
        { 2, 10,  6, 15, 13},
        { 6, 17,  4,  8, 10},
        { 2, 13, 15,  6, 10}, // Will be overridden by clean icosahedron / dodecahedron
        {18,  1, 12,  0, 16},
        { 1,  9,  5, 14, 12},
        { 5, 19, 17,  4, 14},
        { 4, 14, 12,  0,  8}
    ...
    vk_math::Vec3 faceColors[12] = {
        {0.95f, 0.25f, 0.25f}, {0.25f, 0.85f, 0.35f}, {0.25f, 0.45f, 0.95f},
        {0.95f, 0.85f, 0.25f}, {0.85f, 0.25f, 0.95f}, {0.25f, 0.95f, 0.95f},
        {0.95f, 0.55f, 0.15f}, {0.65f, 0.95f, 0.25f}, {0.55f, 0.25f, 0.85f},
        {0.25f, 0.65f, 0.95f}, {0.95f, 0.35f, 0.65f}, {0.45f, 0.85f, 0.55f}
    };

    // Standard canonical dodecahedron face index cycles (each 5-cycle is consecutive along polygon boundary):
    int canonicalFaces[12][5] = {
        { 3, 11,  7, 15, 13},
        { 7, 19, 17,  6, 15},
        { 7, 11,  9,  5, 19},
        {11,  3, 18,  1,  9},
    ...
    for (int f = 0; f < 12; ++f) {
        ...
        for (int i = 0; i < 5; ++i) {
            center.x += rawPts[canonicalFaces[f][i]].x;
            center.y += rawPts[canonicalFaces[f][i]].y;
            center.z += rawPts[canonicalFaces[f][i]].z;
        }
        center.x /= 5.0f;
        center.y /= 5.0f;
        center.z /= 5.0f;
        ...
        vk_math::Vec3 v0 = rawPts[canonicalFaces[f][0]];
        vk_math::Vec3 v1 = rawPts[canonicalFaces[f][1]];
        vk_math::Vec3 v2 = rawPts[canonicalFaces[f][2]];
        vk_math::Vec3 e1(v1.x - v0.x, v1.y - v0.y, v1.z - v0.z);
        vk_math::Vec3 e2(v2.x - v0.x, v2.y - v0.y, v2.z - v0.z);
        vk_math::Vec3 norm(
            e1.y * e2.z - e1.z * e2.y,
            e1.z * e2.x - e1.x * e2.z,
            e1.x * e2.y - e1.y * e2.x
        );
        float dot = norm.x * center.x + norm.y * center.y + norm.z * center.z;
        bool flip = (dot < 0.0f);

        uint16_t cIdx = static_cast<uint16_t>(vertices.size());
        vertices.push_back({ center, faceColors[f] });

        uint16_t faceV[5];
        for (int i = 0; i < 5; ++i) {
            faceV[i] = static_cast<uint16_t>(vertices.size());
            vertices.push_back({ rawPts[canonicalFaces[f][i]], faceColors[f] });
        }
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            if (!flip) {
                ...
                indices.push_back(faceV[i]);
                indices.push_back(faceV[next]);
            } else {
                indices.push_back(cIdx);
                indices.push_back(faceV[next]);
                indices.push_back(faceV[i]);
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 69: AS Compaction & Serialization (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: VK_KHR_acceleration_structure, Compacted BVHs,\n";
    std::cout << "           Serialization & Deserialization, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 69: AS Compaction (Vulkan 1.4)");
        ...
        // Shaders
        std::string vertPath = "assignment69_acceleration_structure_compaction_serialization/shaders/compact_scene.vert.spv";
        std::string fragPath = "assignment69_acceleration_structure_compaction_serialization/shaders/compact_scene.frag.spv";
        if (!std::filesystem::exists(vertPath)) vertPath = "shaders/compact_scene.vert.spv";
        if (!std::filesystem::exists(fragPath)) fragPath = "shaders/compact_scene.frag.spv";
        ...
        generateDodecahedronGeometry(vertices, indices);
        ...
        std::cout << "[Render Loop] Rendering Compacted & Serialized BVH Mesh (VK_KHR_acceleration_structure)...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment69_acceleration_structure_compaction_serialization
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment69_acceleration_structure_compaction_serialization");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment69_acceleration_structure_compaction_serialization");
            glfwPollEvents();
            ...
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.7f, vk_math::Vec3(0.0f, 1.0f, 0.35f)) *
        ...
        profiler.exportFoldedFile("flamegraph_assignment69_acceleration_structure_compaction_serialization.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment69_acceleration_structure_compaction_serialization.html");
        profiler.exportChromeTraceFile("flamegraph_assignment69_acceleration_structure_compaction_serialization.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 69 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 69 Error: " << e.what() << "\n";
        return 1;
    }
