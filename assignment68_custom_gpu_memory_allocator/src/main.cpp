// ============================================================================
// Assignment 68: Custom GPU Memory Sub-Allocator (VK_KHR_buffer_device_address)
// Standardized for Clang 17+ Compiler & Vulkan 1.4 Specification
// Concepts:
//   - Upfront Monolithic GPU Memory Slab Allocation
//   - Custom Free-List Sub-Allocator with Hardware Alignment Constraints
//   - Dynamic Rendering with Depth Testing & Multi-Prism Crystal Array
// ============================================================================
...
// Generate 3D Hexagonal Prism Geometry
void generatePrismGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    ...
    // Center points
    vertices.push_back({ {0.0f,  H, 0.0f}, {1.0f, 1.0f, 1.0f} }); // 0: Top center
    vertices.push_back({ {0.0f, -H, 0.0f}, {0.3f, 0.3f, 0.3f} }); // 1: Bot center

    for (int i = 0; i < SIDES; ++i) {
        float angle = float(i) * 2.0f * vk_math::PI / float(SIDES);
        float x = std::cos(angle) * R;
        float z = std::sin(angle) * R;

        float cr = 0.2f + 0.8f * (float(i) / float(SIDES));
        float cg = 0.9f - 0.5f * (float(i) / float(SIDES));
        float cb = 0.6f;

        vertices.push_back({ {x,  H, z}, {cr, cg, cb} });
        vertices.push_back({ {x, -H, z}, {cr * 0.6f, cg * 0.6f, cb * 0.6f} });
    }

    for (int i = 0; i < SIDES; ++i) {
        uint16_t curTop = static_cast<uint16_t>(2 + i * 2);
        uint16_t curBot = static_cast<uint16_t>(2 + i * 2 + 1);
        uint16_t nextTop = static_cast<uint16_t>(2 + ((i + 1) % SIDES) * 2);
        uint16_t nextBot = static_cast<uint16_t>(2 + ((i + 1) % SIDES) * 2 + 1);

        // Top cap
        indices.push_back(baseV + 0); indices.push_back(baseV + curTop); indices.push_back(baseV + nextTop);
        // Bottom cap
        indices.push_back(baseV + 1); indices.push_back(baseV + nextBot); indices.push_back(baseV + curBot);
        // Side quad
        indices.push_back(baseV + curTop); indices.push_back(baseV + curBot); indices.push_back(baseV + nextBot);
        indices.push_back(baseV + curTop); indices.push_back(baseV + nextBot); indices.push_back(baseV + nextTop);
    }
}
...
// Custom GPU Memory Slab Sub-Allocator
class CustomGpuSlabAllocator {
public:
    void init(VkDevice dev, VkPhysicalDevice physDev, VkDeviceSize slabSize) {
        device = dev;
        totalSize = slabSize;
        currentOffset = 0;
        ...
        allocInfo.allocationSize = slabSize;
        allocInfo.memoryTypeIndex = findMemoryType(physDev, 0xFFFFFFFF, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

        if (vkAllocateMemory(device, &allocInfo, nullptr, &slabMemory) != VK_SUCCESS) {
            allocInfo.pNext = nullptr;
            vk_common::check_vk_result(vkAllocateMemory(device, &allocInfo, nullptr, &slabMemory), "Failed to allocate GPU slab memory");
        }

        vkMapMemory(device, slabMemory, 0, slabSize, 0, &mappedBase);
    }

    void suballocateBuffer(VkDeviceSize size, VkDeviceSize alignment, VkBufferUsageFlags usage,
                           VkBuffer& buffer, VkDeviceSize& outOffset, void*& outMappedPtr) {
        VkDeviceSize alignedOffset = (currentOffset + alignment - 1) & ~(alignment - 1);
        if (alignedOffset + size > totalSize) {
            throw std::runtime_error("GPU Slab Allocator out of memory!");
        }
        ...
        vk_common::check_vk_result(vkCreateBuffer(device, &bufferInfo, nullptr, &buffer), "Failed to create suballocated buffer");

        vk_common::check_vk_result(vkBindBufferMemory(device, buffer, slabMemory, alignedOffset), "Failed to bind suballocated buffer");

        outMappedPtr = static_cast<char*>(mappedBase) + alignedOffset;
    }

    void destroy() {
        if (mappedBase) {
            vkUnmapMemory(device, slabMemory);
            mappedBase = nullptr;
        }
        if (slabMemory != VK_NULL_HANDLE) {
            vkFreeMemory(device, slabMemory, nullptr);
            slabMemory = VK_NULL_HANDLE;
        }
    }
...
int main() {
    std::cout << "========================================================\n";
    std::cout << " Assignment 68: Custom GPU Memory Allocator (Vulkan 1.4)\n";
    std::cout << " Compiled with Clang 17+ Standard | Targeting VK_API_VERSION_1_4\n";
    std::cout << " Concepts: Custom Slab Allocator, Alignment Constraints,\n";
    std::cout << "           Direct Buffer Device Addresses, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 68: Custom GPU Allocator (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/alloc_mesh.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/alloc_mesh.frag.spv");
        ...
        // Custom Slab Allocator Initialization (64MB Dedicated Heap Slab)
        CustomGpuSlabAllocator allocator;
        allocator.init(device, physicalDevice, 64 * 1024 * 1024);

        std::vector<Vertex> vertices;
        std::vector<uint16_t> indices;
        generatePrismGeometry(vertices, indices);

        VkDeviceSize vertexBufferSize = sizeof(vertices[0]) * vertices.size();
        VkBuffer vertexBuffer;
        VkDeviceSize vOffset;
        void* vData;
        allocator.suballocateBuffer(vertexBufferSize, 64, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, vertexBuffer, vOffset, vData);
        ...
        void* iData;
        allocator.suballocateBuffer(indexBufferSize, 64, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, indexBuffer, iOffset, iData);
        ...
        std::cout << "[Render Loop] Rendering Custom Sub-Allocated GPU Buffers...\n";

        auto startTime = std::chrono::high_resolution_clock::now();
        uint64_t frameCount = 0;

        
        // Initialize Flame Graph Profiler for assignment68_custom_gpu_memory_allocator
        auto& profiler = vk_profiler::FlameGraphProfiler::get();
        profiler.setSessionName("assignment68_custom_gpu_memory_allocator");
        profiler.initGpu(device, physicalDevice);



        while (!glfwWindowShouldClose(window)) {
            VK_PROFILE_SCOPE("assignment68_custom_gpu_memory_allocator");
            glfwPollEvents();
            ...
            // Orbiting View
            vk_math::Mat4 model = vk_math::Mat4::rotate(time * 0.75f, vk_math::Vec3(0.0f, 1.0f, 0.4f)) *
                                 vk_math::Mat4::rotate(0.4f, vk_math::Vec3(1.0f, 0.0f, 0.0f));
        ...
        profiler.exportFoldedFile("flamegraph_assignment68_custom_gpu_memory_allocator.folded");
        profiler.exportInteractiveHTML("flamegraph_assignment68_custom_gpu_memory_allocator.html");
        profiler.exportChromeTraceFile("flamegraph_assignment68_custom_gpu_memory_allocator.json");
        profiler.cleanupGpu();
        ...
        std::cout << "\nAssignment 68 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 68 Error: " << e.what() << "\n";
        return 1;
    }
