...
// Concepts:
//   - gl_DrawID & VK_KHR_shader_draw_parameters
//   - Batching 32+ sub-meshes into a single vkCmdDrawIndexedIndirect call
//   - Dynamic Rendering with Depth Testing & Zero-Rebind Architecture
// ============================================================================
...
// Generate 3D Diamond / Polyhedron geometry
void generateDiamondGeometry(std::vector<Vertex>& vertices, std::vector<uint16_t>& indices) {
    uint16_t baseV = static_cast<uint16_t>(vertices.size());
    const float R = 0.5f;

    vertices.push_back({ { 0.0f,  R,  0.0f}, {1.0f, 0.2f, 0.2f} }); // 0: Top
    vertices.push_back({ { 0.0f, -R,  0.0f}, {0.2f, 0.2f, 1.0f} }); // 1: Bot
    vertices.push_back({ { R,  0.0f,  0.0f}, {0.2f, 1.0f, 0.2f} }); // 2: Right
    vertices.push_back({ {-R,  0.0f,  0.0f}, {1.0f, 1.0f, 0.2f} }); // 3: Left
    vertices.push_back({ { 0.0f,  0.0f,  R}, {1.0f, 0.2f, 1.0f} }); // 4: Front
    vertices.push_back({ { 0.0f,  0.0f, -R}, {0.2f, 1.0f, 1.0f} }); // 5: Back

    uint16_t idx[] = {
        0,2,4, 0,4,3, 0,3,5, 0,5,2,
        1,4,2, 1,3,4, 1,5,3, 1,2,5
    };
    for (auto i : idx) indices.push_back(baseV + i);
}
...
int main() {
    ...
    std::cout << " Concepts: gl_DrawID, VK_KHR_shader_draw_parameters,\n";
    std::cout << "           vkCmdDrawIndexedIndirect Multi-Batches, Dynamic Rendering\n";
    std::cout << "========================================================\n";
    ...
    try {
        ...
        GLFWwindow* window = vulkan_utils::createWindow(WIDTH, HEIGHT, "Assignment 63: Multi-Draw Indirect Draw Parameters (Vulkan 1.4)");
        ...
        // Shaders
        auto vertCode = vulkan_utils::readFile("shaders/draw_params.vert.spv");
        auto fragCode = vulkan_utils::readFile("shaders/draw_params.frag.spv");
        ...
        generateDiamondGeometry(vertices, indices);
        ...
        // Multi-Draw Indirect Commands Buffer (32 sub-meshes)
        const uint32_t DRAW_COUNT = 32;
        std::vector<VkDrawIndexedIndirectCommand> indirectCommands(DRAW_COUNT);
        ...
        VkDeviceSize indirectBufferSize = sizeof(VkDrawIndexedIndirectCommand) * DRAW_COUNT;
        ...
        void* cmdData;
        vkMapMemory(device, indirectBufferMemory, 0, indirectBufferSize, 0, &cmdData);
        std::memcpy(cmdData, indirectCommands.data(), (size_t)indirectBufferSize);
        ...
        std::cout << "[Render Loop] Rendering 32 Sub-Meshes via Single vkCmdDrawIndexedIndirect with gl_DrawID...\n";
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            barrierTosubresourceRange: barrierToColor.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 };
            ...
            vk_math::Mat4 rot = vk_math::Mat4::rotate(time * 0.6f, vk_math::Vec3(0.0f, 1.0f, 0.3f));

            PushConstants pc{ proj * view * rot };
            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pc);

            // Single Multi-Draw Indirect Call for all 32 sub-meshes
            vkCmdDrawIndexedIndirect(commandBuffer, indirectBuffer, 0, DRAW_COUNT, sizeof(VkDrawIndexedIndirectCommand));
        ...
        std::cout << "\nAssignment 63 executed cleanly (" << frameCount << " frames rendered).\n";
    } catch (const std::exception& e) {
        std::cerr << "Assignment 63 Error: " << e.what() << "\n";
        return 1;
    }
